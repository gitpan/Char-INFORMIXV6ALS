...
BEGIN {
    ...
    sub Char::Einformixv6als::binmode(*;$);
    sub Char::Einformixv6als::open(*;$@);
    ...
    if ($] < 5.006) {
        ...
        if ($^O ne 'MacOS') {
            *CORE::GLOBAL::binmode =
            *CORE::GLOBAL::binmode = \&Char::Einformixv6als::binmode;
        }
        *CORE::GLOBAL::open    =
        *CORE::GLOBAL::open    = \&Char::Einformixv6als::open;
    }
}
...
if ($^O =~ /\A (?: MSWin32 | NetWare | symbian | dos ) \z/oxms) {
    if ($ENV{'ComSpec'} =~ / (?: COMMAND\.COM | CMD\.EXE ) \z /oxmsi) {
        ...
        for (@ARGV) {
            ...
            elsif (m/\A (?:$q_char)*? [*?] /oxms and (my @glob = Char::Einformixv6als::glob($_))) {
                push @argv, @glob;
            }
            else {
                push @argv, $_;
...
#
{
    ...
    sub Char::Einformixv6als::m_matched() {
        $last_s_matched = 0;
    }

    # INFORMIX V6 ALS regexp mark last s/// or qr matched
    ...
    @Char::Einformixv6als::m_matched = (qr/(?{Char::Einformixv6als::m_matched})/);
    @Char::Einformixv6als::s_matched = (qr/(?{Char::Einformixv6als::s_matched})/);
}
    ...
    for my $string (@string) {
        ...
        for (my $i=0; $i <= $#char; $i++) {
            ...
            # /i modifier
                ...
                my $lc = Char::Einformixv6als::lc($char[$i]);
                if ($uc ne $lc) {
                    $char[$i] = '[' . $uc . $lc . ']';
                }
            }
...
sub _octets {
    ...
    if ($length == 1) {
        ...
        if (((caller(1))[3] ne 'Char::Einformixv6als::_octets') and ($modifier =~ m/i/oxms)) {
            if ($a == $z) {
...
sub Char::Einformixv6als::T(;*@) {
    ...
    local $_ = shift if @_;
    croak 'Too many arguments for -T (Char::Einformixv6als::T)' if @_ and not wantarray;
    my $T = 1;
...
#               $pattern .= "(?:$your_char)",   # UNIX style
            ...
            elsif ((my $uc = Char::Einformixv6als::uc($char)) ne $char) {
                $pattern .= $uc;
            }
            else {
        ...
        my $matchsub = sub { Char::Einformixv6als::uc($_[0]) =~ m{\A $pattern \z}xms };
...
=item Split string
  ...
  Scans a INFORMIX V6 ALS $string for delimiters that match pattern and splits the INFORMIX V6 ALS
  ...
  pattern, so the delimiters may be of any size and need not be the same INFORMIX V6 ALS
  $string on every match. If the pattern doesn't match at all, Char::Einformixv6als::split returns
  the original INFORMIX V6 ALS $string as a single substring. If it matches once, you get
  ...
  If INFORMIX V6 ALS $string is omitted, the function splits the $_ INFORMIX V6 ALS string.
...
=item Chop string
  ...
  Chops off the last character of a INFORMIX V6 ALS string contained in the variable (or
  INFORMIX V6 ALS strings in each element of a @list) and returns the character chopped.
  The Char::Einformixv6als::chop operator is used primarily to remove the newline from the end of
  an input record but is more efficient than s/\n$//. If no argument is given, the
  function chops the $_ variable.

=item Index string

  $pos = Char::Einformixv6als::index($string,$substr,$position);
  $pos = Char::Einformixv6als::index($string,$substr);

  Returns the position of the first occurrence of $substr in INFORMIX V6 ALS $string.
  The start, if specified, specifies the $position to start looking in the INFORMIX V6 ALS
  $string. Positions are integer numbers based at 0. If the substring is not found,
  the Char::Einformixv6als::index function returns -1.

=item Reverse index string

  $pos = Char::Einformixv6als::rindex($string,$substr,$position);
  $pos = Char::Einformixv6als::rindex($string,$substr);

  Works just like Char::Einformixv6als::index except that it returns the position of the last
  occurence of $substr in INFORMIX V6 ALS $string (a reverse index). The function returns
  -1 if not found. $position, if specified, is the rightmost position that may be
  returned, i.e., how far in the INFORMIX V6 ALS string the function can search.
...
=item Make character
  ...
  set. For example, Char::Einformixv6als::chr(65) is "A" in either ASCII or INFORMIX V6 ALS, and
  Char::Einformixv6als::chr(0x82a0) is a INFORMIX V6 ALS HIRAGANA LETTER A. For the reverse of Char::Einformixv6als::chr,
  use Char::INFORMIXV6ALS::ord.

=item File test operator -X

  A file test operator is an unary operator that tests a pathname or a filehandle.
  If $string is omitted, it uses $_ by function Char::Einformixv6als::r_.
  The following functions function when the pathname ends with chr(0x5C) on MSWin32.

  $test = Char::Einformixv6als::r $string;
  $test = Char::Einformixv6als::r_;
  ...
  ------------------------------------------------------------------------------
  Char::Einformixv6als::r(*), Char::Einformixv6als::r_()   File is readable by effective uid/gid
  Char::Einformixv6als::w(*), Char::Einformixv6als::w_()   File is writable by effective uid/gid
  Char::Einformixv6als::x(*), Char::Einformixv6als::x_()   File is executable by effective uid/gid
  Char::Einformixv6als::o(*), Char::Einformixv6als::o_()   File is owned by effective uid
  Char::Einformixv6als::R(*), Char::Einformixv6als::R_()   File is readable by real uid/gid
  Char::Einformixv6als::W(*), Char::Einformixv6als::W_()   File is writable by real uid/gid
  Char::Einformixv6als::X(*), Char::Einformixv6als::X_()   File is executable by real uid/gid
  Char::Einformixv6als::O(*), Char::Einformixv6als::O_()   File is owned by real uid
  Char::Einformixv6als::e(*), Char::Einformixv6als::e_()   File exists
  Char::Einformixv6als::z(*), Char::Einformixv6als::z_()   File has zero size
  Char::Einformixv6als::f(*), Char::Einformixv6als::f_()   File is a plain file
  Char::Einformixv6als::d(*), Char::Einformixv6als::d_()   File is a directory
  Char::Einformixv6als::l(*), Char::Einformixv6als::l_()   File is a symbolic link
  Char::Einformixv6als::p(*), Char::Einformixv6als::p_()   File is a named pipe (FIFO)
  Char::Einformixv6als::S(*), Char::Einformixv6als::S_()   File is a socket
  Char::Einformixv6als::b(*), Char::Einformixv6als::b_()   File is a block special file
  Char::Einformixv6als::c(*), Char::Einformixv6als::c_()   File is a character special file
  Char::Einformixv6als::u(*), Char::Einformixv6als::u_()   File has setuid bit set
  Char::Einformixv6als::g(*), Char::Einformixv6als::g_()   File has setgid bit set
  Char::Einformixv6als::k(*), Char::Einformixv6als::k_()   File has sticky bit set
  ------------------------------------------------------------------------------
  ...
  a filehandle. Both Char::Einformixv6als::T and Char::Einformixv6als::B deosn't work when given the special
  filehandle consisting of a solitary underline.

  Function and Prototype     Meaning
  ------------------------------------------------------------------------------
  Char::Einformixv6als::T(*), Char::Einformixv6als::T_()   File is a text file
  Char::Einformixv6als::B(*), Char::Einformixv6als::B_()   File is a binary file (opposite of -T)
  ------------------------------------------------------------------------------

  Returns useful value if successful, or undef unless successful.

  $value = Char::Einformixv6als::s $string;
  $value = Char::Einformixv6als::s_;

  Function and Prototype     Meaning
  ------------------------------------------------------------------------------
  Char::Einformixv6als::s(*), Char::Einformixv6als::s_()   File has nonzero size (returns size)
  Char::Einformixv6als::M(*), Char::Einformixv6als::M_()   Age of file (at startup) in days since modification
  Char::Einformixv6als::A(*), Char::Einformixv6als::A_()   Age of file (at startup) in days since last access
  Char::Einformixv6als::C(*), Char::Einformixv6als::C_()   Age of file (at startup) in days since inode change
  ------------------------------------------------------------------------------
...
=item Statistics about file
  ...
  @stat = Char::Einformixv6als::stat($file);
  ...
  ($dev,$ino,$mode,$nlink,$uid,$gid,$rdev,$size,
      $atime,$mtime,$ctime,$blksize,$blocks) = Char::Einformixv6als::stat($file);
...
=item require file
  ...
  If the argument is a string, Char::Einformixv6als::require loads and executes the Perl code found in
  the separate file whose name is given by the string. This is similar to performing a
  Char::Einformixv6als::do on a file, except that Char::Einformixv6als::require checks to see whether the library
  ...
  duplicate compilation.) Like its cousins Char::Einformixv6als::do and use, Char::Einformixv6als::require knows how
...
=item binary mode (Perl5.6 emulation on perl5.005)
  ...
  Char::Einformixv6als::binmode(FILEHANDLE, $disciplines);
  Char::Einformixv6als::binmode(FILEHANDLE);
  Char::Einformixv6als::binmode($filehandle, $disciplines);
  Char::Einformixv6als::binmode($filehandle);
...
=item open file (Perl5.6 emulation on perl5.005)
  ...
  $rc = Char::Einformixv6als::open(FILEHANDLE, $mode, $expr);
  $rc = Char::Einformixv6als::open(FILEHANDLE, $expr);
  $rc = Char::Einformixv6als::open(FILEHANDLE);
  $rc = Char::Einformixv6als::open(my $filehandle, $mode, $expr);
  $rc = Char::Einformixv6als::open(my $filehandle, $expr);
  $rc = Char::Einformixv6als::open(my $filehandle);
  ...
  * three arguments
  ...
  {
      my $fh;                          # (uninitialized)
      Char::Einformixv6als::open($fh, ">logfile")     # $fh is autovivified
  ...
  Char::Einformixv6als::open my $fh, ">logfile" or die ...
  ...
  Char::Einformixv6als::open(LOG, ">", "logfile") or die "Can't create logfile: $!";
  ...
  $path =~ s#^([ ])#./$1#;
  Char::Einformixv6als::open (FH, "< $path\0") or die "can't open $path: $!";
  ...
  Char::Einformixv6als::open(FH, "<", $path) or die "can't open $path: $!";
  ...
  Char::Einformixv6als::open(FH, "<:raw", $path) or die "can't open $path: $!";
  ...
  Char::Einformixv6als::open(FH, "<:crlf:encoding(Char::INFORMIXV6ALS)", $path) or die "can't open $path: $!";
