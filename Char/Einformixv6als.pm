...
BEGIN {
    ...
    if ($^O =~ /\A (?: MSWin32 | NetWare | symbian | dos ) \z/oxms) {
        $PERL5LIB =~ s{[^\x81-\x9F\xE0-\xFD/]*$}{Char::INFORMIXV6ALS};
    }
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # POSIX-style character classes
            ...
            $char[$i] = {
                ...
                '[:^lower:]'  => '(?:\xFD[\xA1-\xFE][\xA1-\xFE]|[\x81-\x9F\xE0-\xFC][\x00-\xFF]|[^\x81-\x9F\xE0-\xFD])',
                '[:^upper:]'  => '(?:\xFD[\xA1-\xFE][\xA1-\xFE]|[\x81-\x9F\xE0-\xFC][\x00-\xFF]|[^\x81-\x9F\xE0-\xFD])',
...
=item Make character
  ...
  set. For example, Char::Einformixv6als::chr(65) is "A" in either ASCII or INFORMIX V6 ALS, not Unicode,
  and Char::Einformixv6als::chr(0x82a0) is a INFORMIX V6 ALS HIRAGANA LETTER A. For the reverse of
  Char::Einformixv6als::chr, use Char::INFORMIXV6ALS::ord.
...
=item File test operator -X
  ...
  Function and Prototype     Meaning
  ------------------------------------------------------------------------------
  Char::Einformixv6als::s(*), Char::Einformixv6als::s_()   File has nonzero size (returns size in bytes)
