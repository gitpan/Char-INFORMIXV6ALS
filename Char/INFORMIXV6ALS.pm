...
my $m_matched   = '';
my $s_matched   = '';
$m_matched   = q{@Char::Einformixv6als::m_matched};
...
# split
        ...
        my $e = 'Char::Einformixv6als::split';
...
# chdir
            ...
            my $e_string = 'Char::Einformixv6als::chdir';
...
# split
            ...
            my $e_string = 'Char::Einformixv6als::split';
...
sub character_class {
    ...
    if ($char eq '.') {
        if ($modifier =~ /s/) {
            return '(?:\xFD[\xA1-\xFE][\xA1-\xFE]|[\x81-\x9F\xE0-\xFC][\x00-\xFF]|[^\x81-\x9F\xE0-\xFD])';
        }
        else {
            return '(?:\xFD[\xA1-\xFE][\xA1-\xFE]|[\x81-\x9F\xE0-\xFC][\x00-\xFF]|[^\x81-\x9F\xE0-\xFD\x0A])';
        }
    }
    else {
...
=item * Handles Raw INFORMIX V6 ALS, No UTF8 flag like JPerl
...
The character classes are redefined as follows to backward compatibility.
  ...
  Before        After
  ---------------------------------------------------------------------------
   .            (?:\xFD[\xA1-\xFE][\xA1-\xFE]|[\x81-\x9F\xE0-\xFC][\x00-\xFF]|[^\x81-\x9F\xE0-\xFD\x0A])
                (?:\xFD[\xA1-\xFE][\xA1-\xFE]|[\x81-\x9F\xE0-\xFC][\x00-\xFF]|[^\x81-\x9F\xE0-\xFD])     (/s modifier)
  \d            [0-9]
  \s            [\x09\x0A\x0C\x0D\x20]
  \w            [0-9A-Z_a-z]
  \D            (?:\xFD[\xA1-\xFE][\xA1-\xFE]|[\x81-\x9F\xE0-\xFC][\x00-\xFF]|[^\x81-\x9F\xE0-\xFD0-9])
  \S            (?:\xFD[\xA1-\xFE][\xA1-\xFE]|[\x81-\x9F\xE0-\xFC][\x00-\xFF]|[^\x81-\x9F\xE0-\xFD\x09\x0A\x0C\x0D\x20])
  \W            (?:\xFD[\xA1-\xFE][\xA1-\xFE]|[\x81-\x9F\xE0-\xFC][\x00-\xFF]|[^\x81-\x9F\xE0-\xFD0-9A-Z_a-z])
  \h            [\x09\x20]
  \v            [\x0C\x0A\x0D]
  \H            (?:\xFD[\xA1-\xFE][\xA1-\xFE]|[\x81-\x9F\xE0-\xFC][\x00-\xFF]|[^\x81-\x9F\xE0-\xFD\x09\x20])
  \V            (?:\xFD[\xA1-\xFE][\xA1-\xFE]|[\x81-\x9F\xE0-\xFC][\x00-\xFF]|[^\x81-\x9F\xE0-\xFD\x0C\x0A\x0D])
  \C            [\x00-\xFF]
  \X            X (so, just 'X')
  \R            (?:\x0D\x0A|[\x0A\x0D])
  \N            (?:\xFD[\xA1-\xFE][\xA1-\xFE]|[\x81-\x9F\xE0-\xFC][\x00-\xFF]|[^\x81-\x9F\xE0-\xFD\x0A])
  ---------------------------------------------------------------------------
...
Also POSIX-style character classes.
  ...
  [:^alpha:]    (?:\xFD[\xA1-\xFE][\xA1-\xFE]|[\x81-\x9F\xE0-\xFC][\x00-\xFF]|[^\x81-\x9F\xE0-\xFD\x41-\x5A\x61-\x7A])
  [:^ascii:]    (?:\xFD[\xA1-\xFE][\xA1-\xFE]|[\x81-\x9F\xE0-\xFC][\x00-\xFF]|[^\x81-\x9F\xE0-\xFD\x00-\x7F])
  [:^blank:]    (?:\xFD[\xA1-\xFE][\xA1-\xFE]|[\x81-\x9F\xE0-\xFC][\x00-\xFF]|[^\x81-\x9F\xE0-\xFD\x09\x20])
  [:^cntrl:]    (?:\xFD[\xA1-\xFE][\xA1-\xFE]|[\x81-\x9F\xE0-\xFC][\x00-\xFF]|[^\x81-\x9F\xE0-\xFD\x00-\x1F\x7F])
...
functions.
  ...
  tr///       Char::Einformixv6als::tr
  tr///b      tr///
  tr///B      tr///
  y///        Char::Einformixv6als::tr
  y///b       tr///
  y///B       tr///
  chop        Char::Einformixv6als::chop
  index       Char::Einformixv6als::index
  rindex      Char::Einformixv6als::rindex
  lc          Char::Einformixv6als::lc
  lcfirst     Char::Einformixv6als::lcfirst
  uc          Char::Einformixv6als::uc
  ucfirst     Char::Einformixv6als::ucfirst
  chr         Char::Einformixv6als::chr
  glob        Char::Einformixv6als::glob
  lstat       Char::Einformixv6als::lstat
  opendir     Char::Einformixv6als::opendir
  stat        Char::Einformixv6als::stat
  unlink      Char::Einformixv6als::unlink
  chdir       Char::Einformixv6als::chdir
  do          Char::Einformixv6als::do
  require     Char::Einformixv6als::require
  ---------------------------------
...
=item * Reverse List Or String
  ...
  for (Char::INFORMIXV6ALS::reverse(1 .. 10)) { ... }
  ...
  %barfoo = Char::INFORMIXV6ALS::reverse(%foobar);
...
=item * Special Variables $` And $& Doesn't Function
  ...
  is escaped to
    'AAABBBCCC' =~ /\G(?:\xFD[\xA1-\xFE][\xA1-\xFE]|[\x81-\x9F\xE0-\xFC][\x00-\xFF]|[^\x81-\x9F\xE0-\xFD])*?(?:BBB)@Char::Einformixv6als::m_matched/;
...
page 402 of Programming Perl, 3rd ed. again.
  ...
  Information processing model beginning with this software
    ...
    +-----------------------------------+
    |      ASCII Compatible Encoding    | ex. INFORMIX V6 ALS
    +-----------------------------------+
                (No UTF8 Flag)
