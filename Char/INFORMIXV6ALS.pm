...
sub Char::INFORMIXV6ALS::escape_script {
    ...
    if (m/^ use Char::Einformixv6als(?:\s+[0-9\.]*)?\s*; $/oxms) {
        ...
        if (s/^ \s* use \s+ Char::INFORMIXV6ALS \s* ([^;]*) ; \s* \n? $//oxms) {
...
sub e_use_noimport {
    ...
    for my $realfilename (_realfilename($expr)) {
        ...
        if (CORE::open($fh, $realfilename)) {
            ...
            if ($script =~ m/^ \s* use \s+ Char::INFORMIXV6ALS \s* ([^;]*) ; \s* \n? $/oxms) {
...
=head1 Perl5.6 Emulation On perl5.005
  ...
  --------------------------------------------------------------------
  Before          After                  in BEGIN { } of Char/Einformixv6als.pm
  --------------------------------------------------------------------
  binmode(...);   Char::Einformixv6als::binmode(...);   *CORE::GLOBAL::binmode = ...
  open(...);      Char::Einformixv6als::open(...);      *CORE::GLOBAL::open    = ...
  --------------------------------------------------------------------
...
=item * binmode (Perl5.6 emulation on perl5.005)
  ...
  * two arguments
  ...
  If you are using perl5.005 other than MacPerl, Char::INFORMIXV6ALS software emulate perl5.6's
...
=item * open (Perl5.6 emulation on perl5.005)
  ...
  * three arguments
  ...
  If you are using perl5.005, Char::INFORMIXV6ALS software emulate perl5.6's open function.
  ...
  open(FH, "<:crlf:encoding(Char::INFORMIXV6ALS)", $path) or die "can't open $path: $!";
