...
######################################################################
#
# Char::INFORMIXV6ALS - Source code filter to escape INFORMIX V6 ALS script
#
# http://search.cpan.org/dist/Char-Char::INFORMIXV6ALS/
#
...
BEGIN { CORE::require Char::Einformixv6als; }
...
sub unimport {}
sub Char::INFORMIXV6ALS::escape_script;
...
my $anchor = '';
$anchor = q{${Char::Einformixv6als::anchor}};
...
my $qq_substr  = qr{(?: Char::INFORMIXV6ALS::substr | CORE::substr | substr ) \( $qq_paren \)
                 }xms;
...
my $matched     = '';
my $s_matched   = '';
$matched        = q{$Char::Einformixv6als::matched};
$s_matched      = q{ Char::Einformixv6als::s_matched();};
...
my $function_ord;         # ord()   to ord()   or Char::INFORMIXV6ALS::ord()
my $function_ord_;        # ord     to ord     or Char::INFORMIXV6ALS::ord_
my $function_reverse;     # reverse to reverse or Char::INFORMIXV6ALS::reverse
my $function_getc;        # getc    to getc    or Char::INFORMIXV6ALS::getc
...
END
    ...
    print Char::INFORMIXV6ALS::escape_script($ARGV[0]);
    exit 0;
}
...
sub import {
    ...
    if (Char::Einformixv6als::e("$filename.e")) {
        if (exists $ENV{'SJIS_DEBUG'}) {
            Char::Einformixv6als::unlink "$filename.e";
        }
        elsif (Char::Einformixv6als::z("$filename.e")) {
            Char::Einformixv6als::unlink "$filename.e";
        }
        else {
            my $e_mtime   = (Char::Einformixv6als::stat("$filename.e"))[9];
            my $mtime     = (Char::Einformixv6als::stat($filename))[9];
            my $__mtime__ = (Char::Einformixv6als::stat(__FILE__))[9];
    ...
    if (not Char::Einformixv6als::e("$filename.e")) {
        my $fh = gensym();

        if (eval q{ use Fcntl qw(O_WRONLY O_APPEND O_CREAT); 1 } and CORE::sysopen($fh,"$filename.e",&O_WRONLY|&O_APPEND|&O_CREAT)) {
        }
        else {
            Char::Einformixv6als::_open_a($fh, "$filename.e") or die __FILE__, ": Can't write open file: $filename.e";
        }
        ...
        my $e_script = Char::INFORMIXV6ALS::escape_script($filename);
        print {$fh} $e_script;

        my $mode = (Char::Einformixv6als::stat($filename))[2] & 0777;
    ...
    my $fh = gensym();
    Char::Einformixv6als::_open_r($fh, "$filename.e") or die __FILE__, ": Can't read open file: $filename.e";
    ...
    if ($^O =~ /\A (?: MSWin32 | NetWare | symbian | dos ) \z/oxms) {
        exit Char::Einformixv6als::_systemx(
...
sub Char::INFORMIXV6ALS::escape_script {
    ...
    my $fh = gensym();
    Char::Einformixv6als::_open_r($fh, $script) or die __FILE__, ": Can't open file: $script";
    local $/ = undef; # slurp mode
    $_ = <$fh>;
    close($fh) or die __FILE__, ": Can't close file: $script";

    if (/^ use Char::Einformixv6als(?:\s+[0-9\.]*)?\s*; $/oxms) {
        return $_;
    }
    else {
        ...
        $e_script .= sprintf("use Char::Einformixv6als %s;\n", $Char::INFORMIXV6ALS::VERSION); # require run-time routines version

        # use Char::INFORMIXV6ALS version qw(ord reverse getc);
        $function_ord     = 'ord';
        $function_ord_    = 'ord';
        ...
        if (s/^ \s* use \s+ Char::INFORMIXV6ALS \s* ([^\x81-\x9F\xE0-\xFD;]*) ; \s* \n? $//oxms) {
            ...
            if ($list =~ s/\A ([0-9]+\.[0-9]+) \.0 \s* //oxms) {
                my $version = $1;
                if ($version ne $Char::INFORMIXV6ALS::VERSION) {
                    my @file = grep -e, map {qq{$_/Char/INFORMIXV6ALS.pm}} @INC;
...
****************************************************
              ...
              CONFLICT Char/INFORMIXV6ALS.pm FILE
...
END
                    ...
                    die "Script $0 expects Char/INFORMIXV6ALS.pm $version, but @{[__FILE__]} is version $Char::INFORMIXV6ALS::VERSION\n";
                }
                $e_script .= qq{die "Script \$0 expects Char/Einformixv6als.pm $version, but \\\$Char::Einformixv6als::VERSION is \$Char::Einformixv6als::VERSION" if \$Char::Einformixv6als::VERSION ne '$version';\n};
            }
            elsif ($list =~ s/\A ([0-9]+(?:\.[0-9]*)) \s* //oxms) {
                my $version = $1;
                if ($version > $Char::INFORMIXV6ALS::VERSION) {
                    die "Script $0 required Char/INFORMIXV6ALS.pm $version, but @{[__FILE__]} is only version $Char::INFORMIXV6ALS::VERSION\n";
                }
            }
            ...
            if ($list !~ /\A \s* \z/oxms) {
                ...
                my @list = eval $list;
                for (@list) {
                    $function_ord     = 'Char::INFORMIXV6ALS::ord'     if /\A ord \z/oxms;
                    $function_ord_    = 'Char::INFORMIXV6ALS::ord_'    if /\A ord \z/oxms;
                    $function_reverse = 'Char::INFORMIXV6ALS::reverse' if /\A reverse \z/oxms;
                    $function_getc    = 'Char::INFORMIXV6ALS::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }
...
# $`, ${`}, $PREMATCH, ${PREMATCH}, ${^PREMATCH} --> Char::Einformixv6als::PREMATCH()
    elsif (/\G ( \$` | \$\{`\} | \$ \s* PREMATCH  \b | \$ \s* \{ \s* PREMATCH  \s* \} | \$ \s* \{\^PREMATCH\}  ) /oxmsgc) {
        $slash = 'div';
        return q{Char::Einformixv6als::PREMATCH()};
    }

# $&, ${&}, $MATCH, ${MATCH}, ${^MATCH} --> Char::Einformixv6als::MATCH()
    elsif (/\G ( \$& | \$\{&\} | \$ \s* MATCH     \b | \$ \s* \{ \s* MATCH     \s* \} | \$ \s* \{\^MATCH\}     ) /oxmsgc) {
        $slash = 'div';
        return q{Char::Einformixv6als::MATCH()};
    }

# $POSTMATCH, ${POSTMATCH}, ${^POSTMATCH} --> Char::Einformixv6als::POSTMATCH()
    elsif (/\G (                 \$ \s* POSTMATCH \b | \$ \s* \{ \s* POSTMATCH \s* \} | \$ \s* \{\^POSTMATCH\} ) /oxmsgc) {
        $slash = 'div';
        return q{Char::Einformixv6als::POSTMATCH()};
    }
...
# while (<WILDCARD>) --- glob
    ...
    elsif (/\G \b while \s* \( \s* < ((?:\xFD[\xA1-\xFE][\xA1-\xFE]|[\x81-\x9F\xE0-\xFC][\x00-\xFF]|[^\x81-\x9F\xE0-\xFD>\0\a\e\f\n\r\t])+?) > \s* \) \b /oxgc) {
        return 'while ($_ = Char::Einformixv6als::glob("' . $1 . '"))';
    }

# while (glob)
    elsif (/\G \b while \s* \( \s* glob \s* \) /oxgc) {
        return 'while ($_ = Char::Einformixv6als::glob_)';
    }

# while (glob(WILDCARD))
    elsif (/\G \b while \s* \( \s* glob \b /oxgc) {
        return 'while ($_ = Char::Einformixv6als::glob';
    }
...
# functions of package Char::Einformixv6als
    elsif (/\G \b (CORE:: | ->[ ]* (?: atan2 | [a-z]{2,})) \b  /oxgc) { $slash = 'm//'; return $1;                    }
    elsif (/\G \b bytes::substr \b (?! \s* => )                /oxgc) { $slash = 'm//'; return 'substr';              }
    elsif (/\G \b chop \b          (?! \s* => )                /oxgc) { $slash = 'm//'; return 'Char::Einformixv6als::chop';         }
    elsif (/\G \b bytes::index \b  (?! \s* => )                /oxgc) { $slash = 'm//'; return 'index';               }
    elsif (/\G \b Char::INFORMIXV6ALS::index \b   (?! \s* => )                /oxgc) { $slash = 'm//'; return 'Char::INFORMIXV6ALS::index';         }
    elsif (/\G \b index \b         (?! \s* => )                /oxgc) { $slash = 'm//'; return 'Char::Einformixv6als::index';        }
    elsif (/\G \b bytes::rindex \b (?! \s* => )                /oxgc) { $slash = 'm//'; return 'rindex';              }
    elsif (/\G \b Char::INFORMIXV6ALS::rindex \b  (?! \s* => )                /oxgc) { $slash = 'm//'; return 'Char::INFORMIXV6ALS::rindex';        }
    elsif (/\G \b rindex \b        (?! \s* => )                /oxgc) { $slash = 'm//'; return 'Char::Einformixv6als::rindex';       }
    elsif (/\G \b lc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Char::Einformixv6als::lc';           }
    elsif (/\G \b lcfirst (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Char::Einformixv6als::lcfirst';      }
    elsif (/\G \b uc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Char::Einformixv6als::uc';           }
    elsif (/\G \b ucfirst (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Char::Einformixv6als::ucfirst';      }
    elsif (/\G \b fc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Char::Einformixv6als::fc';           }
    ...
    # (and so on)
                                                         ...
                                                         \s* (\") ((?:$qq_char)+?)             (\") /oxgc) { $slash = 'm//'; return "Char::Einformixv6als::filetest(qw($1)," . e_qq('',  $2,$4,$3) . ")"; }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) qq \s* (\#) ((?:$qq_char)+?)             (\#) /oxgc) { $slash = 'm//'; return "Char::Einformixv6als::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) qq \s* (\() ((?:$qq_paren)+?)            (\)) /oxgc) { $slash = 'm//'; return "Char::Einformixv6als::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) qq \s* (\{) ((?:$qq_brace)+?)            (\}) /oxgc) { $slash = 'm//'; return "Char::Einformixv6als::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) qq \s* (\[) ((?:$qq_bracket)+?)          (\]) /oxgc) { $slash = 'm//'; return "Char::Einformixv6als::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) qq \s* (\<) ((?:$qq_angle)+?)            (\>) /oxgc) { $slash = 'm//'; return "Char::Einformixv6als::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) qq \s* (\S) ((?:$qq_char)+?)             (\3) /oxgc) { $slash = 'm//'; return "Char::Einformixv6als::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; }

    elsif (/\G (-[rwxoRWXOezfdlpSbcugkTB](?:\s+-[rwxoRWXOezfdlpSbcugkTB])+)
                                                         \s* (\') ((?:\\\1|\\\\|$q_char)+?)    (\') /oxgc) { $slash = 'm//'; return "Char::Einformixv6als::filetest(qw($1)," . e_q ('',  $2,$4,$3) . ")"; }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) q  \s* (\#) ((?:\\\#|\\\\|$q_char)+?)    (\#) /oxgc) { $slash = 'm//'; return "Char::Einformixv6als::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) q  \s* (\() ((?:\\\)|\\\\|$q_paren)+?)   (\)) /oxgc) { $slash = 'm//'; return "Char::Einformixv6als::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) q  \s* (\{) ((?:\\\}|\\\\|$q_brace)+?)   (\}) /oxgc) { $slash = 'm//'; return "Char::Einformixv6als::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) q  \s* (\[) ((?:\\\]|\\\\|$q_bracket)+?) (\]) /oxgc) { $slash = 'm//'; return "Char::Einformixv6als::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) q  \s* (\<) ((?:\\\>|\\\\|$q_angle)+?)   (\>) /oxgc) { $slash = 'm//'; return "Char::Einformixv6als::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) q  \s* (\S) ((?:\\\1|\\\\|$q_char)+?)    (\3) /oxgc) { $slash = 'm//'; return "Char::Einformixv6als::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; }

    elsif (/\G (-[rwxoRWXOezfdlpSbcugkTB](?:\s+-[rwxoRWXOezfdlpSbcugkTB])+) (\$ \w+(?: ::\w+)* (?: (?: ->)? (?: \( (?:$qq_paren)*? \) | \{ (?:$qq_brace)+? \} | \[ (?:$qq_bracket)+? \] ) )*) /oxgc)
                                                                                                           { $slash = 'm//'; return "Char::Einformixv6als::filetest(qw($1),$2)"; }
    elsif (/\G (-[rwxoRWXOezfdlpSbcugkTB](?:\s+-[rwxoRWXOezfdlpSbcugkTB])+) \( ((?:$qq_paren)*?) \) /oxgc)
                                                                                                           { $slash = 'm//'; return "Char::Einformixv6als::filetest(qw($1),$2)"; }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) (?= [a-z]+) /oxgc)                                   { $slash = 'm//'; return "Char::Einformixv6als::filetest qw($1),";    }
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) (\w+) /oxgc)                                         { $slash = 'm//'; return "Char::Einformixv6als::filetest(qw($1),$2)"; }

    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+    \s* (\") ((?:$qq_char)+?)             (\") /oxgc)    { $slash = 'm//'; return "Char::Einformixv6als::$1(" . e_qq('',  $2,$4,$3) . ")"; }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ qq \s* (\#) ((?:$qq_char)+?)             (\#) /oxgc)    { $slash = 'm//'; return "Char::Einformixv6als::$1(" . e_qq('qq',$2,$4,$3) . ")"; }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ qq \s* (\() ((?:$qq_paren)+?)            (\)) /oxgc)    { $slash = 'm//'; return "Char::Einformixv6als::$1(" . e_qq('qq',$2,$4,$3) . ")"; }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ qq \s* (\{) ((?:$qq_brace)+?)            (\}) /oxgc)    { $slash = 'm//'; return "Char::Einformixv6als::$1(" . e_qq('qq',$2,$4,$3) . ")"; }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ qq \s* (\[) ((?:$qq_bracket)+?)          (\]) /oxgc)    { $slash = 'm//'; return "Char::Einformixv6als::$1(" . e_qq('qq',$2,$4,$3) . ")"; }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ qq \s* (\<) ((?:$qq_angle)+?)            (\>) /oxgc)    { $slash = 'm//'; return "Char::Einformixv6als::$1(" . e_qq('qq',$2,$4,$3) . ")"; }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ qq \s* (\S) ((?:$qq_char)+?)             (\3) /oxgc)    { $slash = 'm//'; return "Char::Einformixv6als::$1(" . e_qq('qq',$2,$4,$3) . ")"; }

    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+    \s* (\') ((?:\\\1|\\\\|$q_char)+?)    (\') /oxgc)    { $slash = 'm//'; return "Char::Einformixv6als::$1(" . e_q ('',  $2,$4,$3) . ")"; }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ q  \s* (\#) ((?:\\\#|\\\\|$q_char)+?)    (\#) /oxgc)    { $slash = 'm//'; return "Char::Einformixv6als::$1(" . e_q ('q', $2,$4,$3) . ")"; }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ q  \s* (\() ((?:\\\)|\\\\|$q_paren)+?)   (\)) /oxgc)    { $slash = 'm//'; return "Char::Einformixv6als::$1(" . e_q ('q', $2,$4,$3) . ")"; }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ q  \s* (\{) ((?:\\\}|\\\\|$q_brace)+?)   (\}) /oxgc)    { $slash = 'm//'; return "Char::Einformixv6als::$1(" . e_q ('q', $2,$4,$3) . ")"; }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ q  \s* (\[) ((?:\\\]|\\\\|$q_bracket)+?) (\]) /oxgc)    { $slash = 'm//'; return "Char::Einformixv6als::$1(" . e_q ('q', $2,$4,$3) . ")"; }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ q  \s* (\<) ((?:\\\>|\\\\|$q_angle)+?)   (\>) /oxgc)    { $slash = 'm//'; return "Char::Einformixv6als::$1(" . e_q ('q', $2,$4,$3) . ")"; }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ q  \s* (\S) ((?:\\\1|\\\\|$q_char)+?)    (\3) /oxgc)    { $slash = 'm//'; return "Char::Einformixv6als::$1(" . e_q ('q', $2,$4,$3) . ")"; }

    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s* (\$ \w+(?: ::\w+)* (?: (?: ->)? (?: \( (?:$qq_paren)*? \) | \{ (?:$qq_brace)+? \} | \[ (?:$qq_bracket)+? \] ) )*) /oxgc)
                                                                                                           { $slash = 'm//'; return "Char::Einformixv6als::$1($2)";      }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s* \( ((?:$qq_paren)*?) \) /oxgc)                          { $slash = 'm//'; return "Char::Einformixv6als::$1($2)";      }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) (?= \s+ [a-z]+) /oxgc)                                      { $slash = 'm//'; return "Char::Einformixv6als::$1";          }
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ (\w+) /oxgc)                                            { $slash = 'm//'; return "Char::Einformixv6als::$1(::"."$2)"; }
    elsif (/\G -(t)                            \s+ (\w+) /oxgc)                                            { $slash = 'm//'; return "-t $2";              }
    elsif (/\G \b lstat         (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Char::Einformixv6als::lstat';             }
    elsif (/\G \b stat          (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Char::Einformixv6als::stat';              }
    ...
    elsif (/\G \b chr           (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Char::Einformixv6als::chr';               }
    elsif (/\G \b bytes::ord    (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'div'; return 'ord';                      }
    elsif (/\G \b ord           (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'div'; return $function_ord;              }
    elsif (/\G \b glob          (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Char::Einformixv6als::glob';              }
    elsif (/\G \b lc \b         (?! \s* => )                         /oxgc) { $slash = 'm//'; return 'Char::Einformixv6als::lc_';               }
    elsif (/\G \b lcfirst \b    (?! \s* => )                         /oxgc) { $slash = 'm//'; return 'Char::Einformixv6als::lcfirst_';          }
    elsif (/\G \b uc \b         (?! \s* => )                         /oxgc) { $slash = 'm//'; return 'Char::Einformixv6als::uc_';               }
    elsif (/\G \b ucfirst \b    (?! \s* => )                         /oxgc) { $slash = 'm//'; return 'Char::Einformixv6als::ucfirst_';          }
    elsif (/\G \b fc \b         (?! \s* => )                         /oxgc) { $slash = 'm//'; return 'Char::Einformixv6als::fc_';               }
    elsif (/\G    (-[rwxoRWXOezfdlpSbcugkTB](?:\s+-[rwxoRWXOezfdlpSbcugkTB])+)
                           \b    (?! \s* => )                        /oxgc) { $slash = 'm//'; return "Char::Einformixv6als::filetest_(qw($1))"; }
    elsif (/\G    -([rwxoRWXOezsfdlpSbcugkTBMAC])
                           \b    (?! \s* => )                        /oxgc) { $slash = 'm//'; return "Char::Einformixv6als::${1}_";             }
    elsif (/\G \b lstat \b      (?! \s* => )                         /oxgc) { $slash = 'm//'; return 'Char::Einformixv6als::lstat_';            }
    elsif (/\G \b stat \b       (?! \s* => )                         /oxgc) { $slash = 'm//'; return 'Char::Einformixv6als::stat_';             }
    ...
    elsif (/\G \b chr \b           (?! \s* => )                      /oxgc) { $slash = 'm//'; return 'Char::Einformixv6als::chr_';              }
    elsif (/\G \b bytes::ord \b    (?! \s* => )                      /oxgc) { $slash = 'div'; return 'ord';                      }
    elsif (/\G \b ord \b           (?! \s* => )                      /oxgc) { $slash = 'div'; return $function_ord_;             }
    elsif (/\G \b glob \b          (?! \s* => )                      /oxgc) { $slash = 'm//'; return 'Char::Einformixv6als::glob_';             }
    elsif (/\G \b reverse \b       (?! \s* => )                      /oxgc) { $slash = 'm//'; return $function_reverse;          }
    elsif (/\G \b getc \b          (?! \s* => )                      /oxgc) { $slash = 'm//'; return $function_getc;             }
    elsif (/\G \b opendir (\s* \( \s*) (?=[A-Za-z_])                 /oxgc) { $slash = 'm//'; return "Char::Einformixv6als::opendir$1*";        }
    elsif (/\G \b opendir (\s+)        (?=[A-Za-z_])                 /oxgc) { $slash = 'm//'; return "Char::Einformixv6als::opendir$1*";        }
    elsif (/\G \b unlink \b     (?! \s* => )                         /oxgc) { $slash = 'm//'; return 'Char::Einformixv6als::unlink';            }

# chdir
    elsif (/\G \b (chdir) \b    (?! \s* => ) /oxgc) {
        $slash = 'm//';

        my $e = 'Char::Einformixv6als::chdir';
...
# end of split
        if    (/\G (?= [,;\)\}\]] )          /oxgc) { return 'Char::Einformixv6als::split' . $e;                 }

# split scalar value
        elsif (/\G ( [\$\@\&\*] $qq_scalar ) /oxgc) { return 'Char::Einformixv6als::split' . $e . e_string($1);  }

# split literal space
        elsif (/\G \b qq       (\#) [ ] (\#) /oxgc) { return 'Char::Einformixv6als::split' . $e . qq  {qq$1 $2}; }
        elsif (/\G \b qq (\s*) (\() [ ] (\)) /oxgc) { return 'Char::Einformixv6als::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq (\s*) (\{) [ ] (\}) /oxgc) { return 'Char::Einformixv6als::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq (\s*) (\[) [ ] (\]) /oxgc) { return 'Char::Einformixv6als::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq (\s*) (\<) [ ] (\>) /oxgc) { return 'Char::Einformixv6als::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq (\s*) (\S) [ ] (\2) /oxgc) { return 'Char::Einformixv6als::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b q        (\#) [ ] (\#) /oxgc) { return 'Char::Einformixv6als::split' . $e . qq   {q$1 $2}; }
        elsif (/\G \b q  (\s*) (\() [ ] (\)) /oxgc) { return 'Char::Einformixv6als::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  (\s*) (\{) [ ] (\}) /oxgc) { return 'Char::Einformixv6als::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  (\s*) (\[) [ ] (\]) /oxgc) { return 'Char::Einformixv6als::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  (\s*) (\<) [ ] (\>) /oxgc) { return 'Char::Einformixv6als::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  (\s*) (\S) [ ] (\2) /oxgc) { return 'Char::Einformixv6als::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G                ' [ ] '    /oxgc) { return 'Char::Einformixv6als::split' . $e . qq     {' '};  }
        elsif (/\G                " [ ] "    /oxgc) { return 'Char::Einformixv6als::split' . $e . qq     {" "};  }
...
# do
    ...
    elsif (/\G \b do (?= \s+ (?: q|qq|qx) \b)        /oxmsgc)                                  { return 'Char::Einformixv6als::do';         }
    elsif (/\G \b do (?= \s+ \w+)                    /oxmsgc)                                  { return 'do';                }
    elsif (/\G \b do (?= \s* \$ \w+ (?: ::\w+)* \( ) /oxmsgc)                                  { return 'do';                }
    elsif (/\G \b do \b                              /oxmsgc)                                  { return 'Char::Einformixv6als::do';         }
...
# require else
    elsif (/\G \b require                                   \s* ; /oxmsgc)                     { return 'Char::Einformixv6als::require;';   }
    elsif (/\G \b require \b                                      /oxmsgc)                     { return 'Char::Einformixv6als::require';    }
...
# <WILDCARD> --- glob
    ...
    elsif (/\G < ((?:\xFD[\xA1-\xFE][\xA1-\xFE]|[\x81-\x9F\xE0-\xFC][\x00-\xFF]|[^\x81-\x9F\xE0-\xFD>\0\a\e\f\n\r\t])+?) > /oxgc) {
        return 'Char::Einformixv6als::glob("' . $1 . '")';
    }
...
# $`, ${`}, $PREMATCH, ${PREMATCH}, ${^PREMATCH} --> @{[Char::Einformixv6als::PREMATCH()]}
            ...
            $e_string .= q{Char::Einformixv6als::PREMATCH()};
            $slash = 'div';
        }

# $&, ${&}, $MATCH, ${MATCH}, ${^MATCH} --> @{[Char::Einformixv6als::MATCH()]}
        elsif ($string =~ /\G ( \$& | \$\{&\} | \$ \s* MATCH     \b | \$ \s* \{ \s* MATCH     \s* \} | \$ \s* \{\^MATCH\}     ) /oxmsgc) {
            $e_string .= q{Char::Einformixv6als::MATCH()};
            $slash = 'div';
        }

# $POSTMATCH, ${POSTMATCH}, ${^POSTMATCH} --> @{[Char::Einformixv6als::POSTMATCH()]}
        elsif ($string =~ /\G (                 \$ \s* POSTMATCH \b | \$ \s* \{ \s* POSTMATCH \s* \} | \$ \s* \{\^POSTMATCH\} ) /oxmsgc) {
            $e_string .= q{Char::Einformixv6als::POSTMATCH()};
            $slash = 'div';
        }
...
# functions of package Char::Einformixv6als
        ...
        elsif ($string =~ /\G \b chop \b                                      /oxgc) { $e_string .= 'Char::Einformixv6als::chop';    $slash = 'm//'; }
        elsif ($string =~ /\G \b bytes::index \b                              /oxgc) { $e_string .= 'index';          $slash = 'm//'; }
        elsif ($string =~ /\G \b Char::INFORMIXV6ALS::index \b                               /oxgc) { $e_string .= 'Char::INFORMIXV6ALS::index';    $slash = 'm//'; }
        elsif ($string =~ /\G \b index \b                                     /oxgc) { $e_string .= 'Char::Einformixv6als::index';   $slash = 'm//'; }
        elsif ($string =~ /\G \b bytes::rindex \b                             /oxgc) { $e_string .= 'rindex';         $slash = 'm//'; }
        elsif ($string =~ /\G \b Char::INFORMIXV6ALS::rindex \b                              /oxgc) { $e_string .= 'Char::INFORMIXV6ALS::rindex';   $slash = 'm//'; }
        elsif ($string =~ /\G \b rindex \b                                    /oxgc) { $e_string .= 'Char::Einformixv6als::rindex';  $slash = 'm//'; }
        elsif ($string =~ /\G \b lc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Char::Einformixv6als::lc';      $slash = 'm//'; }
        elsif ($string =~ /\G \b lcfirst (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Char::Einformixv6als::lcfirst'; $slash = 'm//'; }
        elsif ($string =~ /\G \b uc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Char::Einformixv6als::uc';      $slash = 'm//'; }
        elsif ($string =~ /\G \b ucfirst (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Char::Einformixv6als::ucfirst'; $slash = 'm//'; }
        elsif ($string =~ /\G \b fc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Char::Einformixv6als::fc';      $slash = 'm//'; }
        elsif ($string =~ /\G (-[rwxoRWXOezfdlpSbcugkTB](?:\s+-[rwxoRWXOezfdlpSbcugkTB])+)
                                                                        \s* (\") ((?:$qq_char)+?)             (\") /oxgc) { $e_string .= "Char::Einformixv6als::filetest(qw($1)," . e_qq('',  $2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) qq \s* (\#) ((?:$qq_char)+?)             (\#) /oxgc) { $e_string .= "Char::Einformixv6als::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) qq \s* (\() ((?:$qq_paren)+?)            (\)) /oxgc) { $e_string .= "Char::Einformixv6als::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) qq \s* (\{) ((?:$qq_brace)+?)            (\}) /oxgc) { $e_string .= "Char::Einformixv6als::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) qq \s* (\[) ((?:$qq_bracket)+?)          (\]) /oxgc) { $e_string .= "Char::Einformixv6als::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) qq \s* (\<) ((?:$qq_angle)+?)            (\>) /oxgc) { $e_string .= "Char::Einformixv6als::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) qq \s* (\S) ((?:$qq_char)+?)             (\3) /oxgc) { $e_string .= "Char::Einformixv6als::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; $slash = 'm//'; }

        elsif ($string =~ /\G (-[rwxoRWXOezfdlpSbcugkTB](?:\s+-[rwxoRWXOezfdlpSbcugkTB])+)
                                                                        \s* (\') ((?:\\\1|\\\\|$q_char)+?)    (\') /oxgc) { $e_string .= "Char::Einformixv6als::filetest(qw($1)," . e_q ('',  $2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) q  \s* (\#) ((?:\\\#|\\\\|$q_char)+?)    (\#) /oxgc) { $e_string .= "Char::Einformixv6als::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) q  \s* (\() ((?:\\\)|\\\\|$q_paren)+?)   (\)) /oxgc) { $e_string .= "Char::Einformixv6als::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) q  \s* (\{) ((?:\\\}|\\\\|$q_brace)+?)   (\}) /oxgc) { $e_string .= "Char::Einformixv6als::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) q  \s* (\[) ((?:\\\]|\\\\|$q_bracket)+?) (\]) /oxgc) { $e_string .= "Char::Einformixv6als::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) q  \s* (\<) ((?:\\\>|\\\\|$q_angle)+?)   (\>) /oxgc) { $e_string .= "Char::Einformixv6als::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) q  \s* (\S) ((?:\\\1|\\\\|$q_char)+?)    (\3) /oxgc) { $e_string .= "Char::Einformixv6als::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; $slash = 'm//'; }

        elsif ($string =~ /\G (-[rwxoRWXOezfdlpSbcugkTB](?:\s+-[rwxoRWXOezfdlpSbcugkTB])+) (\$ \w+(?: ::\w+)* (?: (?: ->)? (?: \( (?:$qq_paren)*? \) | \{ (?:$qq_brace)+? \} | \[ (?:$qq_bracket)+? \] ) )*) /oxgc)
                                                                                                                          { $e_string .= "Char::Einformixv6als::filetest(qw($1),$2)"; $slash = 'm//'; }
        elsif ($string =~ /\G (-[rwxoRWXOezfdlpSbcugkTB](?:\s+-[rwxoRWXOezfdlpSbcugkTB])+) \( ((?:$qq_paren)*?) \) /oxgc)
                                                                                                                          { $e_string .= "Char::Einformixv6als::filetest(qw($1),$2)"; $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) (?= [a-z]+) /oxgc)                                   { $e_string .= "Char::Einformixv6als::filetest qw($1),";    $slash = 'm//'; }
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) (\w+) /oxgc)                                         { $e_string .= "Char::Einformixv6als::filetest(qw($1),$2)"; $slash = 'm//'; }

        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+    \s* (\") ((?:$qq_char)+?)             (\") /oxgc)    { $e_string .= "Char::Einformixv6als::$1(" . e_qq('',  $2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ qq \s* (\#) ((?:$qq_char)+?)             (\#) /oxgc)    { $e_string .= "Char::Einformixv6als::$1(" . e_qq('qq',$2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ qq \s* (\() ((?:$qq_paren)+?)            (\)) /oxgc)    { $e_string .= "Char::Einformixv6als::$1(" . e_qq('qq',$2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ qq \s* (\{) ((?:$qq_brace)+?)            (\}) /oxgc)    { $e_string .= "Char::Einformixv6als::$1(" . e_qq('qq',$2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ qq \s* (\[) ((?:$qq_bracket)+?)          (\]) /oxgc)    { $e_string .= "Char::Einformixv6als::$1(" . e_qq('qq',$2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ qq \s* (\<) ((?:$qq_angle)+?)            (\>) /oxgc)    { $e_string .= "Char::Einformixv6als::$1(" . e_qq('qq',$2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ qq \s* (\S) ((?:$qq_char)+?)             (\3) /oxgc)    { $e_string .= "Char::Einformixv6als::$1(" . e_qq('qq',$2,$4,$3) . ")"; $slash = 'm//'; }

        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+    \s* (\') ((?:\\\1|\\\\|$q_char)+?)    (\') /oxgc)    { $e_string .= "Char::Einformixv6als::$1(" . e_q ('',  $2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ q  \s* (\#) ((?:\\\#|\\\\|$q_char)+?)    (\#) /oxgc)    { $e_string .= "Char::Einformixv6als::$1(" . e_q ('q', $2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ q  \s* (\() ((?:\\\)|\\\\|$q_paren)+?)   (\)) /oxgc)    { $e_string .= "Char::Einformixv6als::$1(" . e_q ('q', $2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ q  \s* (\{) ((?:\\\}|\\\\|$q_brace)+?)   (\}) /oxgc)    { $e_string .= "Char::Einformixv6als::$1(" . e_q ('q', $2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ q  \s* (\[) ((?:\\\]|\\\\|$q_bracket)+?) (\]) /oxgc)    { $e_string .= "Char::Einformixv6als::$1(" . e_q ('q', $2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ q  \s* (\<) ((?:\\\>|\\\\|$q_angle)+?)   (\>) /oxgc)    { $e_string .= "Char::Einformixv6als::$1(" . e_q ('q', $2,$4,$3) . ")"; $slash = 'm//'; }
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ q  \s* (\S) ((?:\\\1|\\\\|$q_char)+?)    (\3) /oxgc)    { $e_string .= "Char::Einformixv6als::$1(" . e_q ('q', $2,$4,$3) . ")"; $slash = 'm//'; }

        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s* (\$ \w+(?: ::\w+)* (?: (?: ->)? (?: \( (?:$qq_paren)*? \) | \{ (?:$qq_brace)+? \} | \[ (?:$qq_bracket)+? \] ) )*) /oxgc)
                                                                                                                          { $e_string .= "Char::Einformixv6als::$1($2)";      $slash = 'm//'; }
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s* \( ((?:$qq_paren)*?) \) /oxgc)                          { $e_string .= "Char::Einformixv6als::$1($2)";      $slash = 'm//'; }
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) (?= \s+ [a-z]+) /oxgc)                                      { $e_string .= "Char::Einformixv6als::$1";          $slash = 'm//'; }
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ (\w+) /oxgc)                                            { $e_string .= "Char::Einformixv6als::$1(::"."$2)"; $slash = 'm//'; }
        elsif ($string =~ /\G -(t)                            \s+ (\w+) /oxgc)                                            { $e_string .= "-t $2";              $slash = 'm//'; }
        elsif ($string =~ /\G \b lstat         (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Char::Einformixv6als::lstat';             $slash = 'm//'; }
        elsif ($string =~ /\G \b stat          (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Char::Einformixv6als::stat';              $slash = 'm//'; }
        ...
        elsif ($string =~ /\G \b chr           (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Char::Einformixv6als::chr';               $slash = 'm//'; }
        ...
        elsif ($string =~ /\G \b glob          (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Char::Einformixv6als::glob';              $slash = 'm//'; }
        elsif ($string =~ /\G \b lc \b                                              /oxgc) { $e_string .= 'Char::Einformixv6als::lc_';               $slash = 'm//'; }
        elsif ($string =~ /\G \b lcfirst \b                                         /oxgc) { $e_string .= 'Char::Einformixv6als::lcfirst_';          $slash = 'm//'; }
        elsif ($string =~ /\G \b uc \b                                              /oxgc) { $e_string .= 'Char::Einformixv6als::uc_';               $slash = 'm//'; }
        elsif ($string =~ /\G \b ucfirst \b                                         /oxgc) { $e_string .= 'Char::Einformixv6als::ucfirst_';          $slash = 'm//'; }
        elsif ($string =~ /\G \b fc \b                                              /oxgc) { $e_string .= 'Char::Einformixv6als::fc_';               $slash = 'm//'; }
        elsif ($string =~ /\G    (-[rwxoRWXOezfdlpSbcugkTB](?:\s+-[rwxoRWXOezfdlpSbcugkTB])+)
                                                                 \b                 /oxgc) { $e_string .= "Char::Einformixv6als::filetest_(qw($1))"; $slash = 'm//'; }
        elsif ($string =~ /\G    -([rwxoRWXOezsfdlpSbcugkTBMAC]) \b                 /oxgc) { $e_string .= "Char::Einformixv6als::${1}_";             $slash = 'm//'; }
        elsif ($string =~ /\G \b lstat \b                                           /oxgc) { $e_string .= 'Char::Einformixv6als::lstat_';            $slash = 'm//'; }
        elsif ($string =~ /\G \b stat \b                                            /oxgc) { $e_string .= 'Char::Einformixv6als::stat_';             $slash = 'm//'; }
        ...
        elsif ($string =~ /\G \b chr \b                                             /oxgc) { $e_string .= 'Char::Einformixv6als::chr_';              $slash = 'm//'; }
        elsif ($string =~ /\G \b bytes::ord \b                                      /oxgc) { $e_string .= 'ord';                      $slash = 'div'; }
        elsif ($string =~ /\G \b ord \b                                             /oxgc) { $e_string .= $function_ord_;             $slash = 'div'; }
        elsif ($string =~ /\G \b glob \b                                            /oxgc) { $e_string .= 'Char::Einformixv6als::glob_';             $slash = 'm//'; }
        ...
        elsif ($string =~ /\G \b opendir (\s* \( \s*) (?=[A-Za-z_])                 /oxgc) { $e_string .= "Char::Einformixv6als::opendir$1*";        $slash = 'm//'; }
        elsif ($string =~ /\G \b opendir (\s+)        (?=[A-Za-z_])                 /oxgc) { $e_string .= "Char::Einformixv6als::opendir$1*";        $slash = 'm//'; }
        elsif ($string =~ /\G \b unlink \b                                          /oxgc) { $e_string .= 'Char::Einformixv6als::unlink';            $slash = 'm//'; }

# chdir
        elsif ($string =~ /\G \b (chdir) \b (?! \s* => ) /oxgc) {
            $slash = 'm//';

            $e_string .= 'Char::Einformixv6als::chdir';
...
# end of split
            if    ($string =~ /\G (?= [,;\)\}\]] )          /oxgc) { return 'Char::Einformixv6als::split' . $e;                                           }

# split scalar value
            elsif ($string =~ /\G ( [\$\@\&\*] $qq_scalar ) /oxgc) { $e_string .= 'Char::Einformixv6als::split' . $e . e_string($1);  next E_STRING_LOOP; }

# split literal space
            elsif ($string =~ /\G \b qq       (\#) [ ] (\#) /oxgc) { $e_string .= 'Char::Einformixv6als::split' . $e . qq  {qq$1 $2}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq (\s*) (\() [ ] (\)) /oxgc) { $e_string .= 'Char::Einformixv6als::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq (\s*) (\{) [ ] (\}) /oxgc) { $e_string .= 'Char::Einformixv6als::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq (\s*) (\[) [ ] (\]) /oxgc) { $e_string .= 'Char::Einformixv6als::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq (\s*) (\<) [ ] (\>) /oxgc) { $e_string .= 'Char::Einformixv6als::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq (\s*) (\S) [ ] (\2) /oxgc) { $e_string .= 'Char::Einformixv6als::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q        (\#) [ ] (\#) /oxgc) { $e_string .= 'Char::Einformixv6als::split' . $e . qq   {q$1 $2}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  (\s*) (\() [ ] (\)) /oxgc) { $e_string .= 'Char::Einformixv6als::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  (\s*) (\{) [ ] (\}) /oxgc) { $e_string .= 'Char::Einformixv6als::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  (\s*) (\[) [ ] (\]) /oxgc) { $e_string .= 'Char::Einformixv6als::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  (\s*) (\<) [ ] (\>) /oxgc) { $e_string .= 'Char::Einformixv6als::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  (\s*) (\S) [ ] (\2) /oxgc) { $e_string .= 'Char::Einformixv6als::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G                ' [ ] '    /oxgc) { $e_string .= 'Char::Einformixv6als::split' . $e . qq     {' '};  next E_STRING_LOOP; }
            elsif ($string =~ /\G                " [ ] "    /oxgc) { $e_string .= 'Char::Einformixv6als::split' . $e . qq     {" "};  next E_STRING_LOOP; }
...
# <WILDCARD>   --- glob
            ...
            $e_string .= 'Char::Einformixv6als::glob("' . $1 . '")';
        }
...
sub character_class {
    ...
    if ($char eq '.') {
        if ($modifier =~ /s/) {
            return '${Char::Einformixv6als::dot_s}';
        }
        else {
            return '${Char::Einformixv6als::dot}';
        }
    }
    else {
        return Char::Einformixv6als::classic_character_class($char);
    }
}
...
sub e_capture {
    ...
    return join '', '${Char::Einformixv6als::capture(', $_[0], ')}';
    return join '', '$',                 $_[0];
}
...
sub e_tr {
    ...
    if ($modifier =~ tr/bB//d) {
        ...
        if ($variable eq '') {
            $e_tr = qq{Char::Einformixv6als::tr(\$_,' =~ ',$charclass,$e$charclass2,'$modifier')};
        }
        else {
            $e_tr = qq{Char::Einformixv6als::tr($variable,'$bind_operator',$charclass,$e$charclass2,'$modifier')};
        }
    }
...
sub e_qq {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # octal escape sequence
            ...
            $char[$i] = Char::Einformixv6als::octchr($1);
        }

        # hexadecimal escape sequence
        elsif ($char[$i] =~ /\A \\x \{ ([0-9A-Fa-f]+) \} \z/oxms) {
            $char[$i] = Char::Einformixv6als::hexchr($1);
        }
        ...
        # \u \l \U \L \F \Q \E
            ...
            $char[$i] = '@{[Char::Einformixv6als::ucfirst qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\l') {
            $char[$i] = '@{[Char::Einformixv6als::lcfirst qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\U') {
            $char[$i] = '@{[Char::Einformixv6als::uc qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\L') {
            $char[$i] = '@{[Char::Einformixv6als::lc qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\F') {
            $char[$i] = '@{[Char::Einformixv6als::fc qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\Q') {
...
sub e_qr {
    ...
    my @char = $string =~ /\G(
        ...
        \\c   [\x40-\x5F]          |
        \\N\{ [^\x81-\x9F\xE0-\xFD0-9\}][^\x81-\x9F\xE0-\xFD\}]* \} |
        \\p\{ [^\x81-\x9F\xE0-\xFD0-9\}][^\x81-\x9F\xE0-\xFD\}]* \} |
        \\P\{ [^\x81-\x9F\xE0-\xFD0-9\}][^\x81-\x9F\xE0-\xFD\}]* \} |
        \\  (?:$q_char)            |
        \$` | \$\{`\} | \$ \s* PREMATCH  | \$ \s* \{ \s* PREMATCH  \s* \} | \$ \s* \{\^PREMATCH\}  |
        \$& | \$\{&\} | \$ \s* MATCH     | \$ \s* \{ \s* MATCH     \s* \} | \$ \s* \{\^MATCH\}     |
    ...
    for (my $i=0; $i <= $#char; $i++) {
            ...
            while (1) {
                ...
                if ($char[$i] eq ']') {
                    ...
                    if (grep(/\A [\$\@]/oxms,@char[$left+1..$right-1]) >= 1) {
                        splice @char, $left, $right-$left+1, sprintf(q{@{[Char::Einformixv6als::charlist_qr(%s,'%s')]}}, join(',', map {qq_stuff($delimiter,$end_delimiter,$_)} @char[$left+1..$right-1]), $modifier);
                    }
                    else {
                        splice @char, $left, $right-$left+1, Char::Einformixv6als::charlist_qr(@char[$left+1..$right-1], $modifier);
                    }
        ...
        # open character class [^...]
            ...
            while (1) {
                ...
                if ($char[$i] eq ']') {
                    ...
                    # [^...]
                    if (grep(/\A [\$\@]/oxms,@char[$left+1..$right-1]) >= 1) {
                        splice @char, $left, $right-$left+1, sprintf(q{@{[Char::Einformixv6als::charlist_not_qr(%s,'%s')]}}, join(',', map {qq_stuff($delimiter,$end_delimiter,$_)} @char[$left+1..$right-1]), $modifier);
                    }
                    else {
                        splice @char, $left, $right-$left+1, Char::Einformixv6als::charlist_not_qr(@char[$left+1..$right-1], $modifier);
                    }
        ...
        # /i modifier
        elsif ($ignorecase and ($char[$i] =~ /\A [\x00-\xFF] \z/oxms) and (Char::Einformixv6als::uc($char[$i]) ne Char::Einformixv6als::fc($char[$i]))) {
            if (CORE::length(Char::Einformixv6als::fc($char[$i])) == 1) {
                $char[$i] = '['   . Char::Einformixv6als::uc($char[$i])       . Char::Einformixv6als::fc($char[$i]) . ']';
            }
            else {
                $char[$i] = '(?:' . Char::Einformixv6als::uc($char[$i]) . '|' . Char::Einformixv6als::fc($char[$i]) . ')';
            }
        ...
        # $0 --> $0
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Char::Einformixv6als::ignorecase(' . $char[$i] . ')]}';
            }
        }
        elsif ($char[$i] =~ /\A \$ \{ \s* 0 \s* \} \z/oxms) {
            if ($ignorecase) {
        ...
        # $`, ${`}, $PREMATCH, ${PREMATCH}, ${^PREMATCH} --> Char::Einformixv6als::PREMATCH()
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Char::Einformixv6als::ignorecase(Char::Einformixv6als::PREMATCH())]}';
            }
            else {
                $char[$i] = '@{[Char::Einformixv6als::PREMATCH()]}';
            }
        ...
        # $&, ${&}, $MATCH, ${MATCH}, ${^MATCH} --> Char::Einformixv6als::MATCH()
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Char::Einformixv6als::ignorecase(Char::Einformixv6als::MATCH())]}';
            }
            else {
                $char[$i] = '@{[Char::Einformixv6als::MATCH()]}';
            }
        ...
        # $POSTMATCH, ${POSTMATCH}, ${^POSTMATCH} --> Char::Einformixv6als::POSTMATCH()
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Char::Einformixv6als::ignorecase(Char::Einformixv6als::POSTMATCH())]}';
            }
            else {
                $char[$i] = '@{[Char::Einformixv6als::POSTMATCH()]}';
            }
...
sub e_sub {
    ...
    if ($r_modifier) {
        ...
        # s///gr with multibyte anchoring
            ...
            $sub = sprintf(
                #                        1                                              2   3                                 4   5
                q<eval{local $Char::INFORMIXV6ALS::re_t=%s; local $Char::INFORMIXV6ALS::re_a=''; while($Char::INFORMIXV6ALS::re_t =~ %s){%s local $^W=0; local $Char::INFORMIXV6ALS::re_r=%s; %s$Char::INFORMIXV6ALS::re_t="$Char::INFORMIXV6ALS::re_a${1}$Char::INFORMIXV6ALS::re_r$'"; pos($Char::INFORMIXV6ALS::re_t)=length "$Char::INFORMIXV6ALS::re_a${1}$Char::INFORMIXV6ALS::re_r"; $Char::INFORMIXV6ALS::re_a=substr($Char::INFORMIXV6ALS::re_t,0,pos($Char::INFORMIXV6ALS::re_t)); } return $Char::INFORMIXV6ALS::re_t}>,
                ...
                $s_matched,                                                      #  3
                $e_replacement,                                                  #  4
                '$Char::INFORMIXV6ALS::re_r=eval $Char::INFORMIXV6ALS::re_r; ' x $e_modifier,                  #  5
            );
        }
        ...
        # s///gr without multibyte anchoring
            ...
            $sub = sprintf(
                #                        1                        2   3                                 4   5
                q<eval{local $Char::INFORMIXV6ALS::re_t=%s; while($Char::INFORMIXV6ALS::re_t =~ %s){%s local $^W=0; local $Char::INFORMIXV6ALS::re_r=%s; %s$Char::INFORMIXV6ALS::re_t="$`$Char::INFORMIXV6ALS::re_r$'"; pos($Char::INFORMIXV6ALS::re_t)=length "$`$Char::INFORMIXV6ALS::re_r"; } return $Char::INFORMIXV6ALS::re_t}>,
        ...
        # s///r
            ...
            $sub = sprintf(
                #  1     2          3                                 4   5  6                    7
                q<(%s =~ %s) ? eval{%s local $^W=0; local $Char::INFORMIXV6ALS::re_r=%s; %s"%s$Char::INFORMIXV6ALS::re_r$'" } : %s>,
    ...
    # without /r
        ...
        # s///g with multibyte anchoring
            ...
            $sub = sprintf(
                ...
                q<eval{local $Char::INFORMIXV6ALS::re_n=0; local $Char::INFORMIXV6ALS::re_a=''; while(%s =~ %s){%s local $^W=0; local $Char::INFORMIXV6ALS::re_r=%s; %s%s="$Char::INFORMIXV6ALS::re_a${1}$Char::INFORMIXV6ALS::re_r$'"; pos(%s)=length "$Char::INFORMIXV6ALS::re_a${1}$Char::INFORMIXV6ALS::re_r"; $Char::INFORMIXV6ALS::re_a=substr(%s,0,pos(%s)); $Char::INFORMIXV6ALS::re_n++} return %s$Char::INFORMIXV6ALS::re_n}>,
...
# (and so on)
        ...
        # s///g without multibyte anchoring
            ...
            $sub = sprintf(
                #                                 1     2   3                                 4   5 6                         7                                                 8
                q<eval{local $Char::INFORMIXV6ALS::re_n=0; while(%s =~ %s){%s local $^W=0; local $Char::INFORMIXV6ALS::re_r=%s; %s%s="$`$Char::INFORMIXV6ALS::re_r$'"; pos(%s)=length "$`$Char::INFORMIXV6ALS::re_r"; $Char::INFORMIXV6ALS::re_n++} return %s$Char::INFORMIXV6ALS::re_n}>,
        ...
        # s///
            ...
            $sub = sprintf(
                ...
                #  1 2 3          4                                 5   6 7   8
                q<(%s%s%s) ? eval{%s local $^W=0; local $Char::INFORMIXV6ALS::re_r=%s; %s%s="%s$Char::INFORMIXV6ALS::re_r$'"; 1 } : undef> :

                #  1 2 3              4                                 5   6 7   8
                q<(%s%s%s) ? 1 : eval{%s local $^W=0; local $Char::INFORMIXV6ALS::re_r=%s; %s%s="%s$Char::INFORMIXV6ALS::re_r$'"; undef }>,
                ...
                $s_matched,                                                      #  4
                $e_replacement,                                                  #  5
                '$Char::INFORMIXV6ALS::re_r=eval $Char::INFORMIXV6ALS::re_r; ' x $e_modifier,                  #  6
                $variable,                                                       #  7
                $prematch,                                                       #  8
            );
        }
...
sub e_chdir_q {
    ...
    if ($^W) {
        if (Char::Einformixv6als::_MSWin32_5Cended_path($string)) {
            if ($] !~ /^5\.005/) {
                warn <<END;
...
sub e_split {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # ${ foo }
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Char::Einformixv6als::ignorecase(' . $1 . ')]}';
            }
        }
    ...
    $modifier =~ tr/i//d;
    if ($left_e > $right_e) {
        return join '', 'Char::Einformixv6als::split', $ope, $delimiter, @char, '>]}' x ($left_e - $right_e), $end_delimiter, $modifier;
    }
    return     join '', 'Char::Einformixv6als::split', $ope, $delimiter, @char,                               $end_delimiter, $modifier;
}
    ...
    for my $realfilename (_realfilename($expr)) {
        ...
        if (Char::Einformixv6als::_open_r($fh, $realfilename)) {
            ...
            if ($script =~ /^ \s* use \s+ Char::INFORMIXV6ALS \s* ([^\x81-\x9F\xE0-\xFD;]*) ; \s* \n? $/oxms) {
                ...
                return qq[BEGIN { Char::Einformixv6als::require '$expr'; $module->import() if $module->can('import'); }];
            }
            last;
        }
    }
...
sub e_no_noparam {
    ...
    for my $realfilename (_realfilename($expr)) {
        ...
        if (Char::Einformixv6als::_open_r($fh, $realfilename)) {
            ...
            if ($script =~ /^ \s* use \s+ Char::INFORMIXV6ALS \s* ([^\x81-\x9F\xE0-\xFD;]*) ; \s* \n? $/oxms) {
                return qq[BEGIN { Char::Einformixv6als::require '$expr'; $module->unimport() if $module->can('unimport'); }];
            }
            last;
        }
    }
...
sub e_use {
    ...
    for my $realfilename (_realfilename($expr)) {
        ...
        if (Char::Einformixv6als::_open_r($fh, $realfilename)) {
            ...
            if ($script =~ /^ \s* use \s+ Char::INFORMIXV6ALS \s* ([^\x81-\x9F\xE0-\xFD;]*) ; \s* \n? $/oxms) {
                return qq[BEGIN { Char::Einformixv6als::require '$expr'; $module->import($list) if $module->can('import'); }];
            }
            last;
        }
    }
...
sub e_no {
    ...
    for my $realfilename (_realfilename($expr)) {
        ...
        if (Char::Einformixv6als::_open_r($fh, $realfilename)) {
            ...
            if ($script =~ /^ \s* use \s+ Char::INFORMIXV6ALS \s* ([^\x81-\x9F\xE0-\xFD;]*) ; \s* \n? $/oxms) {
                return qq[BEGIN { Char::Einformixv6als::require '$expr'; $module->unimport($list) if $module->can('unimport'); }];
            }
            last;
        }
    }
...
=item * You'll have to download Char/INFORMIXV6ALS.pm and Char/Einformixv6als.pm and put it in your perl lib directory.

=item * You'll need to write "use Char::INFORMIXV6ALS;" at head of the script.
...
=head1 SYNOPSIS
  ...
  use Char::INFORMIXV6ALS ver.sion;             --- require minimum version
  use Char::INFORMIXV6ALS ver.sion.0;           --- expects version (match or die)
  use Char::INFORMIXV6ALS qw(ord reverse getc); --- demand enhanced feature of ord, reverse, and getc
  use Char::INFORMIXV6ALS ver.sion qw(ord reverse getc);
  use Char::INFORMIXV6ALS ver.sion.0 qw(ord reverse getc);

  # "no Char::INFORMIXV6ALS;" not supported

  or

  $ perl Char/INFORMIXV6ALS.pm INFORMIX V6 ALS_script.pl > Escaped_script.pl.e
  ...
  functions:
    Char::INFORMIXV6ALS::ord(...);
    Char::INFORMIXV6ALS::reverse(...);
    Char::INFORMIXV6ALS::getc(...);
    Char::INFORMIXV6ALS::length(...);
    Char::INFORMIXV6ALS::substr(...);
    Char::INFORMIXV6ALS::index(...);
    Char::INFORMIXV6ALS::rindex(...);
    <*>
    glob(...);
    CORE::chop(...);
    CORE::ord(...);
...
Char::INFORMIXV6ALS software is "middleware" between perl interpreter and your Perl script
...
By "use Char::INFORMIXV6ALS;", it automatically interpret your script as INFORMIX V6 ALS. The various
...
=head1 JRE: JPerl Runtime Environment
  ...
  +---------------------------------------+
  |  Source Code Filter, Runtime Routine  | ex. Char/INFORMIXV6ALS.pm, Char/Einformixv6als.pm
  +---------------------------------------+
...
expand wildcard arguments supplied to programs. Instead, import of Char/Einformixv6als.pm
works well.

   in Char/Einformixv6als.pm
   #
...
=head1 Software Composition
   ...
   Char/INFORMIXV6ALS.pm               --- source code filter to escape INFORMIX V6 ALS
   Char/Einformixv6als.pm              --- run-time routines for Char/INFORMIXV6ALS.pm
...
You need write 'use Char::INFORMIXV6ALS;' in your script.

  ---------------------------------
  Before      You do
  ---------------------------------
  (nothing)   use Char::INFORMIXV6ALS;
  ---------------------------------

=head1 Escaping Multiple-Octet Code (Char/INFORMIXV6ALS.pm provides)
...
=head1 Multiple-Octet Anchoring of Regular Expression (Char/INFORMIXV6ALS.pm provides)

Char/INFORMIXV6ALS.pm applies multiple-octet anchoring at beginning of regular expression.

  --------------------------------------------------------------------------------
  Before                  After
  --------------------------------------------------------------------------------
  m/regexp/               m/${Char::Einformixv6als::anchor}(?:regexp).../
  --------------------------------------------------------------------------------

=head1 Escaping Second Octet (Char/INFORMIXV6ALS.pm provides)

Char/INFORMIXV6ALS.pm escapes second octet of multiple-octet character in regular expression.
...
=head1 Multiple-Octet Character Regular Expression (Char/INFORMIXV6ALS.pm provides)

Char/INFORMIXV6ALS.pm clusters multiple-octet character with quantifier, makes cluster from
...
from classic Perl character class shortcuts and POSIX-style character classes.
  ...
  m/...\D.../             m/...(?:${Char::Einformixv6als::eD}).../
  m/...[[:^digit:]].../   m/...(?:${Char::Einformixv6als::not_digit}).../
  --------------------------------------------------------------------------------

=head1 Calling 'Char::Einformixv6als::ignorecase()' (Char/INFORMIXV6ALS.pm provides)

Char/INFORMIXV6ALS.pm applies calling 'Char::Einformixv6als::ignorecase()' instead of /i modifier.

  --------------------------------------------------------------------------------
  Before                  After
  --------------------------------------------------------------------------------
  m/...$var.../i          m/...@{[Char::Einformixv6als::ignorecase($var)]}.../
  --------------------------------------------------------------------------------
...
Regular expression works as character-oriented that has no /b modifier.
  ...
  Before                  After
  --------------------------------------------------------------------------------
  /regexp/                /ditto$Char::Einformixv6als::matched/
  m/regexp/               m/ditto$Char::Einformixv6als::matched/
  ?regexp?                m?ditto$Char::Einformixv6als::matched?
  m?regexp?               m?ditto$Char::Einformixv6als::matched?
 
  $_ =~                   ($_ =~ m/ditto$Char::Einformixv6als::matched/) ?
  s/regexp/replacement/   eval{ Char::Einformixv6als::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
 
  $_ !~                   ($_ !~ m/ditto$Char::Einformixv6als::matched/) ?
  s/regexp/replacement/   1 :
                          eval{ Char::Einformixv6als::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; undef }
 
  split(/regexp/)         Char::Einformixv6als::split(qr/regexp/)
  split(m/regexp/)        Char::Einformixv6als::split(qr/regexp/)
  split(qr/regexp/)       Char::Einformixv6als::split(qr/regexp/)
  qr/regexp/              qr/ditto$Char::Einformixv6als::matched/
  --------------------------------------------------------------------------------
...
Regular expression works as byte-oriented that has /b modifier.
  ...
  Before                  After
  --------------------------------------------------------------------------------
  /regexp/b               /(?:regexp)$Char::Einformixv6als::matched/
  m/regexp/b              m/(?:regexp)$Char::Einformixv6als::matched/
  ?regexp?b               m?regexp$Char::Einformixv6als::matched?
  m?regexp?b              m?regexp$Char::Einformixv6als::matched?
 
  $_ =~                   ($_ =~ m/(\G[\x00-\xFF]*?)(?:regexp)$Char::Einformixv6als::matched/) ?
  s/regexp/replacement/b  eval{ Char::Einformixv6als::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
  ...
  qr/regexp/b             qr/(?:regexp)$Char::Einformixv6als::matched/
  --------------------------------------------------------------------------------

=head1 Escaping Character Classes (Char/Einformixv6als.pm provides)
...
The character classes are redefined as follows to backward compatibility.
  ...
  Before        After
  ---------------------------------------------------------------
   .            ${Char::Einformixv6als::dot}
                ${Char::Einformixv6als::dot_s}    (/s modifier)
  \d            [0-9]
  \s            \s
  \w            [0-9A-Z_a-z]
...
Also POSIX-style character classes.
  ...
  [:^lower:]    ${Char::Einformixv6als::not_lower}
                ${Char::Einformixv6als::not_lower_i}    (/i modifier)
  ...
  [:^upper:]    ${Char::Einformixv6als::not_upper}
                ${Char::Einformixv6als::not_upper_i}    (/i modifier)
...
Definitions in Char/Einformixv6als.pm.

  ---------------------------------------------------------------------------------------------------------------------------------------------------------
  After                    Definition
  ---------------------------------------------------------------------------------------------------------------------------------------------------------
  ${Char::Einformixv6als::anchor}         qr{\G(?:\xFD[\xA1-\xFE][\xA1-\xFE]|[\x81-\x9F\xE0-\xFC][\x00-\xFF]|[^\x81-\x9F\xE0-\xFD])*?}
...
=head1 Un-Escaping \ of \N, \p, \P and \X (Char/INFORMIXV6ALS.pm provides)

Char/INFORMIXV6ALS.pm removes '\' at head of alphanumeric regexp metasymbols \N, \p, \P
...
=head1 Escaping Built-in Functions (Char/INFORMIXV6ALS.pm and Char/Einformixv6als.pm provide)

Insert 'Char::Einformixv6als::' at head of function name. Char/Einformixv6als.pm provides your script Char::Einformixv6als::*
functions.
  ...
  substr      substr           Byte
  pos         pos              Byte
  split       Char::Einformixv6als::split     Character
  tr///       Char::Einformixv6als::tr        Character
  tr///b      tr///            Byte
  tr///B      tr///            Byte
  y///        Char::Einformixv6als::tr        Character
  y///b       tr///            Byte
  y///B       tr///            Byte
  chop        Char::Einformixv6als::chop      Character
  index       Char::Einformixv6als::index     Character
  rindex      Char::Einformixv6als::rindex    Character
  lc          Char::Einformixv6als::lc        Character
  lcfirst     Char::Einformixv6als::lcfirst   Character
  uc          Char::Einformixv6als::uc        Character
  ucfirst     Char::Einformixv6als::ucfirst   Character
  fc          Char::Einformixv6als::fc        Character
  chr         Char::Einformixv6als::chr       Character
  glob        Char::Einformixv6als::glob      Character
  lstat       Char::Einformixv6als::lstat     Character
  opendir     Char::Einformixv6als::opendir   Character
  stat        Char::Einformixv6als::stat      Character
  unlink      Char::Einformixv6als::unlink    Character
  chdir       Char::Einformixv6als::chdir     Character
  do          Char::Einformixv6als::do        Character
  require     Char::Einformixv6als::require   Character
  -------------------------------------------

  ------------------------------------------------------------------------------------------------------------------------
  Before                   After
  ------------------------------------------------------------------------------------------------------------------------
  use Perl::Module;        BEGIN { Char::Einformixv6als::require 'Perl/Module.pm'; Perl::Module->import() if Perl::Module->can('import'); }
  use Perl::Module @list;  BEGIN { Char::Einformixv6als::require 'Perl/Module.pm'; Perl::Module->import(@list) if Perl::Module->can('import'); }
  use Perl::Module ();     BEGIN { Char::Einformixv6als::require 'Perl/Module.pm'; }
  no Perl::Module;         BEGIN { Char::Einformixv6als::require 'Perl/Module.pm'; Perl::Module->unimport() if Perl::Module->can('unimport'); }
  no Perl::Module @list;   BEGIN { Char::Einformixv6als::require 'Perl/Module.pm'; Perl::Module->unimport(@list) if Perl::Module->can('unimport'); }
  no Perl::Module ();      BEGIN { Char::Einformixv6als::require 'Perl/Module.pm'; }
  ------------------------------------------------------------------------------------------------------------------------

=head1 Escaping File Test Operators (Char/INFORMIXV6ALS.pm and Char/Einformixv6als.pm provide)

Insert 'Char::Einformixv6als::' instead of '-' of operator.

  Available in MSWin32, MacOS, and UNIX-like systems
  --------------------------------------------------------------------------
  Before   After      Meaning
  --------------------------------------------------------------------------
  -r       Char::Einformixv6als::r   File or directory is readable by this (effective) user or group
  -w       Char::Einformixv6als::w   File or directory is writable by this (effective) user or group
  -e       Char::Einformixv6als::e   File or directory name exists
  -x       Char::Einformixv6als::x   File or directory is executable by this (effective) user or group
  -z       Char::Einformixv6als::z   File exists and has zero size (always false for directories)
  -f       Char::Einformixv6als::f   Entry is a plain file
  -d       Char::Einformixv6als::d   Entry is a directory
  -t       -t         The filehandle is a TTY (as reported by the isatty() system function;
                      filenames can't be tested by this test)
  -T       Char::Einformixv6als::T   File looks like a "text" file
  -B       Char::Einformixv6als::B   File looks like a "binary" file
  -M       Char::Einformixv6als::M   Modification age (measured in days)
  -A       Char::Einformixv6als::A   Access age (measured in days)
  -C       Char::Einformixv6als::C   Inode-modification age (measured in days)
  -s       Char::Einformixv6als::s   File or directory exists and has nonzero size
  ...
  Before   After      Meaning
  --------------------------------------------------------------------------
  -R       Char::Einformixv6als::R   File or directory is readable by this real user or group
  -W       Char::Einformixv6als::W   File or directory is writable by this real user or group
  -X       Char::Einformixv6als::X   File or directory is executable by this real user or group
  -l       Char::Einformixv6als::l   Entry is a symbolic link
  -S       Char::Einformixv6als::S   Entry is a socket
  --------------------------------------------------------------------------
  ...
  Before   After      Meaning
  --------------------------------------------------------------------------
  -o       Char::Einformixv6als::o   File or directory is owned by this (effective) user
  -O       Char::Einformixv6als::O   File or directory is owned by this real user
  -p       Char::Einformixv6als::p   Entry is a named pipe (a "fifo")
  -b       Char::Einformixv6als::b   Entry is a block-special file (like a mountable disk)
  -c       Char::Einformixv6als::c   Entry is a character-special file (like an I/O device)
  -u       Char::Einformixv6als::u   File or directory is setuid
  -g       Char::Einformixv6als::g   File or directory is setgid
  -k       Char::Einformixv6als::k   File or directory has the sticky bit set
  --------------------------------------------------------------------------
...
You need write 'Char::INFORMIXV6ALS::' at head of function name when you want character-
oriented function. See 'Character-Oriented Functions'.

  --------------------------------------------------------
  Function   Character-Oriented   Description
  --------------------------------------------------------
  ord        Char::INFORMIXV6ALS::ord
  reverse    Char::INFORMIXV6ALS::reverse
  getc       Char::INFORMIXV6ALS::getc
  length     Char::INFORMIXV6ALS::length
  substr     Char::INFORMIXV6ALS::substr
  index      Char::INFORMIXV6ALS::index          See 'About Indexes'
  rindex     Char::INFORMIXV6ALS::rindex         See 'About Rindexes'
  --------------------------------------------------------
  ...
  (same as Char::Einformixv6als::index)
  Char::INFORMIXV6ALS::index    Character   Character    Character-oriented semantics
  ...
  (same as Char::Einformixv6als::rindex)
  Char::INFORMIXV6ALS::rindex   Character   Character    Character-oriented semantics
...
=item * Ordinal Value of Character
  ...
  $ord = Char::INFORMIXV6ALS::ord($string);

  This function returns the numeric value (ASCII or INFORMIX V6 ALS character) of the
  first character of $string, not Unicode. If $string is omitted, it uses $_.
  The return value is always unsigned.

  If you import ord "use Char::INFORMIXV6ALS qw(ord);", ord of your script will be rewritten in
  Char::INFORMIXV6ALS::ord. Char::INFORMIXV6ALS::ord is not compatible with ord of JPerl.

=item * Reverse List or String

  @reverse = Char::INFORMIXV6ALS::reverse(@list);
  $reverse = Char::INFORMIXV6ALS::reverse(@list);
  ...
  If you import reverse "use Char::INFORMIXV6ALS qw(reverse);", reverse of your script will be
  rewritten in Char::INFORMIXV6ALS::reverse. Char::INFORMIXV6ALS::reverse is not compatible with reverse of
  JPerl.
...
=item * Returns Next Character
  ...
  $getc = Char::INFORMIXV6ALS::getc(FILEHANDLE);
  $getc = Char::INFORMIXV6ALS::getc($filehandle);
  $getc = Char::INFORMIXV6ALS::getc;
  ...
  If you import getc "use Char::INFORMIXV6ALS qw(getc);", getc of your script will be rewritten in
  Char::INFORMIXV6ALS::getc. Char::INFORMIXV6ALS::getc is not compatible with getc of JPerl.

=item * Length by INFORMIX V6 ALS Character

  $length = Char::INFORMIXV6ALS::length($string);
  $length = Char::INFORMIXV6ALS::length();

  This function returns the length in characters (programmer-visible characters) of
  the scalar value $string. If $string is omitted, it returns the Char::INFORMIXV6ALS::length of
  $_.

  Do not try to use Char::INFORMIXV6ALS::length to find the size of an array or hash. Use scalar
...
=item * Substr by INFORMIX V6 ALS Character
  ...
  $substr = Char::INFORMIXV6ALS::substr($string,$offset,$length,$replacement);
  $substr = Char::INFORMIXV6ALS::substr($string,$offset,$length);
  $substr = Char::INFORMIXV6ALS::substr($string,$offset);
  ...
  my $color  = Char::INFORMIXV6ALS::substr $s, 4, 5;      # black
  my $middle = Char::INFORMIXV6ALS::substr $s, 4, -11;    # black cat climbed the
  my $end    = Char::INFORMIXV6ALS::substr $s, 14;        # climbed the green tree
  my $tail   = Char::INFORMIXV6ALS::substr $s, -4;        # tree
  my $z      = Char::INFORMIXV6ALS::substr $s, -4, 2;     # tr

  If Perl version 5.14 or later, you can use the Char::INFORMIXV6ALS::substr() function as an
  ...
  end of the string, Char::INFORMIXV6ALS::substr() returns the undefined value and produces a
  ...
  my $name = 'fred';
  Char::INFORMIXV6ALS::substr($name, 4) = 'dy';         # $name is now 'freddy'
  my $null = Char::INFORMIXV6ALS::substr $name, 6, 2;   # returns "" (no warning)
  my $oops = Char::INFORMIXV6ALS::substr $name, 7;      # returns undef, with warning
  Char::INFORMIXV6ALS::substr($name, 7) = 'gap';        # raises an exception

  An alternative to using Char::INFORMIXV6ALS::substr() as an lvalue is to specify the replacement
  ...
  my $s = "The black cat climbed the green tree";
  my $z = Char::INFORMIXV6ALS::substr $s, 14, 7, "jumped from";    # climbed
  # $s is now "The black cat jumped from the green tree"

  Note that the lvalue returned by the three-argument version of Char::INFORMIXV6ALS::substr() acts
  ...
  $x = '1234';
  for (Char::INFORMIXV6ALS::substr($x,1,2)) {
      $_ = 'a';   print $x,"\n";    # prints 1a4
      $_ = 'xyz'; print $x,"\n";    # prints 1xyz4
      $x = '56789';
      $_ = 'pq';  print $x,"\n";    # prints 5pq9
  ...
  $x = '1234';
  for (Char::INFORMIXV6ALS::substr($x, -3, 2)) {
      $_ = 'a';   print $x,"\n";    # prints 1a4, as above
      $x = 'abcdefg';
      print $_,"\n";                # prints f
  }
...
=item * Index by INFORMIX V6 ALS Character
  ...
  $index = Char::INFORMIXV6ALS::index($string,$substring,$offset);
  $index = Char::INFORMIXV6ALS::index($string,$substring);
  ...
  $pos = -1;
  while (($pos = Char::INFORMIXV6ALS::index($string, $lookfor, $pos)) > -1) {
      print "Found at $pos\n";
      $pos++;
  }
...
=item * Rindex by INFORMIX V6 ALS Character
  ...
  $rindex = Char::INFORMIXV6ALS::rindex($string,$substring,$offset);
  $rindex = Char::INFORMIXV6ALS::rindex($string,$substring);

  This function works just like Char::INFORMIXV6ALS::index except that it returns the character
  position of the last occurrence of $substring in $string (a reverse Char::INFORMIXV6ALS::index).
  ...
  $pos = Char::INFORMIXV6ALS::length($string);
  while (($pos = Char::INFORMIXV6ALS::rindex($string, $lookfor, $pos)) >= 0) {
      print "Found at $pos\n";
      $pos--;
  }
...
=item * Filename Globbing
  ...
  This operator is implemented via the Char::Einformixv6als::glob() function. See Char::Einformixv6als::glob of
  Char/Einformixv6als.pm for details.
...
=item * Ordinal Value of Byte
  ...
  of "use Char::INFORMIXV6ALS qw(ord);" exists or not. If $expr is omitted, it uses $_.
  The return value is always unsigned.
...
=item * Reverse List or Byte String
  ...
  "use Char::INFORMIXV6ALS qw(reverse);" exists or not.
...
=head1 Un-Escaping bytes::* Functions (Char/INFORMIXV6ALS.pm provides)

Char/INFORMIXV6ALS.pm removes 'bytes::' at head of function name.
...
=head1 Escaping Built-in Standard Module (Char/Einformixv6als.pm provides)

Char/Einformixv6als.pm does "BEGIN { unshift @INC, '/Perl/site/lib/Char::INFORMIXV6ALS' }" at head.
Store the standard module modified for Char::INFORMIXV6ALS software in this directory to
override built-in standard modules.

=head1 Escaping Standard Module Content (You do)

You need copy built-in standard module to /Perl/site/lib/Char::INFORMIXV6ALS and change
'use utf8;' to 'use Char::INFORMIXV6ALS;' in its. You need help yourself for now.
...
=head1 Ignore Pragmas and Modules
  ...
  Comment out pragma to ignore utf8 environment, and Char/Einformixv6als.pm provides these
  functions.
...
=item * Char::INFORMIXV6ALS::substr as Lvalue

Char::INFORMIXV6ALS::substr differs from CORE::substr, and cannot be used as a lvalue.
To change part of a string, you can use the optional fourth argument which is the
replacement string.

Char::INFORMIXV6ALS::substr($string, 13, 4, "JPerl");
...
=item * Special Variables $` and $& need /( Capture All )/
  ...
  Before          After                Works as
  -------------------------------------------------------------------------------------------
  $`              Char::Einformixv6als::PREMATCH()    CORE::substr($&,0,CORE::length($&)-CORE::length($1))
  $PREMATCH       Char::Einformixv6als::PREMATCH()    CORE::substr($&,0,CORE::length($&)-CORE::length($1))
  ${^PREMATCH}    Char::Einformixv6als::PREMATCH()    CORE::substr($&,0,CORE::length($&)-CORE::length($1))
  $&              Char::Einformixv6als::MATCH()       $1
  $MATCH          Char::Einformixv6als::MATCH()       $1
  ${^MATCH}       Char::Einformixv6als::MATCH()       $1
  $'              Char::Einformixv6als::POSTMATCH()   $'
  $POSTMATCH      Char::Einformixv6als::POSTMATCH()   $'
  ${^POSTMATCH}   Char::Einformixv6als::POSTMATCH()   $'
  -------------------------------------------------------------------------------------------
...
Even if ${^WIN32_SLOPPY_STAT} is set to a true value, file test functions Char::Einformixv6als::*(),
Char::Einformixv6als::lstat(), and Char::Einformixv6als::stat() on Microsoft Windows open the file for the path
...
exist at time of be written this document and JPerl did exist.
                        ...
                        (a)     (b)     (c)     (d)     (e)
                                      JPerl           Encode,Char::INFORMIXV6ALS
      +--------------+-------+-------+-------+-------+-------+
      | data         |  Old  |  Old  |  New  |  Old  |  New  |
      +--------------+-------+-------+-------+-------+-------+
      | script       |  Old  |      Old      |      New      |
...
Char::INFORMIXV6ALS software remains one language and one interpreter.
