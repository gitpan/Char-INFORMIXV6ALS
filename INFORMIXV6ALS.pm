...
sub e_qr {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # /i modifier
        elsif ($ignorecase and ($char[$i] =~ m/\A [\x00-\xFF] \z/oxms) and (Einformixv6als::uc($char[$i]) ne Einformixv6als::lc($char[$i]))) {
            $char[$i] = '[' . Einformixv6als::uc($char[$i]) . Einformixv6als::lc($char[$i]) . ']';
        }
