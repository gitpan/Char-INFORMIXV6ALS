...
my $qq_substr  = qr{(?: INFORMIXV6ALS::substr | CORE::substr | substr ) \( $qq_paren \)
                 }xms;
...
my $function_getc;        # getc    to getc    or INFORMIXV6ALS::getc
...
END
            ...
            if ($list !~ /\A \s* \z/oxms) {
                ...
                my @list = eval $list;
                for (@list) {
                    $function_ord     = 'INFORMIXV6ALS::ord'     if /\A ord \z/oxms;
                    $function_ord_    = 'INFORMIXV6ALS::ord_'    if /\A ord \z/oxms;
                    $function_reverse = 'INFORMIXV6ALS::reverse' if /\A reverse \z/oxms;
                    $function_getc    = 'INFORMIXV6ALS::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }
...
# functions of package Einformixv6als
    ...
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) qq \s* (\S) ((?:$qq_char)+?)             (\3) /oxgc) { $slash = 'm//'; return "Einformixv6als::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; }
    ...
    elsif (/\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) q  \s* (\S) ((?:\\\1|\\\\|$q_char)+?)    (\3) /oxgc) { $slash = 'm//'; return "Einformixv6als::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; }
    ...
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+    \s* (\") ((?:$qq_char)+?)             (\") /oxgc)    { $slash = 'm//'; return "Einformixv6als::$1(" . e_qq('',  $2,$4,$3) . ")"; }
    ...
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ qq \s* (\S) ((?:$qq_char)+?)             (\3) /oxgc)    { $slash = 'm//'; return "Einformixv6als::$1(" . e_qq('qq',$2,$4,$3) . ")"; }

    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+    \s* (\') ((?:\\\1|\\\\|$q_char)+?)    (\') /oxgc)    { $slash = 'm//'; return "Einformixv6als::$1(" . e_q ('',  $2,$4,$3) . ")"; }
    ...
    elsif (/\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ q  \s* (\S) ((?:\\\1|\\\\|$q_char)+?)    (\3) /oxgc)    { $slash = 'm//'; return "Einformixv6als::$1(" . e_q ('q', $2,$4,$3) . ")"; }
...
# functions of package Einformixv6als
        ...
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) qq \s* (\S) ((?:$qq_char)+?)             (\3) /oxgc) { $e_string .= "Einformixv6als::filetest(qw($1)," . e_qq('qq',$2,$4,$3) . ")"; $slash = 'm//'; }
        ...
        elsif ($string =~ /\G ((?:-[rwxoRWXOezfdlpSbcugkTB]\s+){2,}) q  \s* (\S) ((?:\\\1|\\\\|$q_char)+?)    (\3) /oxgc) { $e_string .= "Einformixv6als::filetest(qw($1)," . e_q ('q', $2,$4,$3) . ")"; $slash = 'm//'; }
        ...
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+    \s* (\") ((?:$qq_char)+?)             (\") /oxgc)    { $e_string .= "Einformixv6als::$1(" . e_qq('',  $2,$4,$3) . ")"; $slash = 'm//'; }
        ...
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ qq \s* (\S) ((?:$qq_char)+?)             (\3) /oxgc)    { $e_string .= "Einformixv6als::$1(" . e_qq('qq',$2,$4,$3) . ")"; $slash = 'm//'; }

        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+    \s* (\') ((?:\\\1|\\\\|$q_char)+?)    (\') /oxgc)    { $e_string .= "Einformixv6als::$1(" . e_q ('',  $2,$4,$3) . ")"; $slash = 'm//'; }
        ...
        elsif ($string =~ /\G -([rwxoRWXOezsfdlpSbcugkTBMAC]) \s+ q  \s* (\S) ((?:\\\1|\\\\|$q_char)+?)    (\3) /oxgc)    { $e_string .= "Einformixv6als::$1(" . e_q ('q', $2,$4,$3) . ")"; $slash = 'm//'; }
    ...
    if ($r_modifier) {
        ...
        # s///gr with multibyte anchoring
            ...
            $sub = sprintf(
                ...
                $s_matched,                                                      #  3
                $e_replacement,                                                  #  4
                '$INFORMIXV6ALS::re_r=eval $INFORMIXV6ALS::re_r; ' x $e_modifier,                  #  5
            );
        }
        ...
        # s///gr without multibyte anchoring
            ...
            $sub = sprintf(
                #                        1                        2   3                                 4   5
                q<eval{local $INFORMIXV6ALS::re_t=%s; while($INFORMIXV6ALS::re_t =~ %s){%s local $^W=0; local $INFORMIXV6ALS::re_r=%s; %s$INFORMIXV6ALS::re_t="$`$INFORMIXV6ALS::re_r$'"; pos($INFORMIXV6ALS::re_t)=length "$`$INFORMIXV6ALS::re_r"; } return $INFORMIXV6ALS::re_t}>,
        ...
        # s///r
            ...
            $sub = sprintf(
                #  1     2          3                                 4   5  6                    7
                q<(%s =~ %s) ? eval{%s local $^W=0; local $INFORMIXV6ALS::re_r=%s; %s"%s$INFORMIXV6ALS::re_r$'" } : %s>,
    ...
    # without /r
        ...
        # s///g with multibyte anchoring
            ...
            $sub = sprintf(
                ...
                q<eval{local $INFORMIXV6ALS::re_n=0; local $INFORMIXV6ALS::re_a=''; while(%s =~ %s){%s local $^W=0; local $INFORMIXV6ALS::re_r=%s; %s%s="$INFORMIXV6ALS::re_a${1}$INFORMIXV6ALS::re_r$'"; pos(%s)=length "$INFORMIXV6ALS::re_a${1}$INFORMIXV6ALS::re_r"; $INFORMIXV6ALS::re_a=substr(%s,0,pos(%s)); $INFORMIXV6ALS::re_n++} return %s$INFORMIXV6ALS::re_n}>,
...
# (and so on)
        ...
        # s///g without multibyte anchoring
            ...
            $sub = sprintf(
                #                                 1     2   3                                 4   5 6                         7                                                 8
                q<eval{local $INFORMIXV6ALS::re_n=0; while(%s =~ %s){%s local $^W=0; local $INFORMIXV6ALS::re_r=%s; %s%s="$`$INFORMIXV6ALS::re_r$'"; pos(%s)=length "$`$INFORMIXV6ALS::re_r"; $INFORMIXV6ALS::re_n++} return %s$INFORMIXV6ALS::re_n}>,
        ...
        # s///
            ...
            $sub = sprintf(
                ...
                #  1 2 3          4                                 5   6 7   8
                q<(%s%s%s) ? eval{%s local $^W=0; local $INFORMIXV6ALS::re_r=%s; %s%s="%s$INFORMIXV6ALS::re_r$'"; 1 } : undef> :

                #  1 2 3              4                                 5   6 7   8
                q<(%s%s%s) ? 1 : eval{%s local $^W=0; local $INFORMIXV6ALS::re_r=%s; %s%s="%s$INFORMIXV6ALS::re_r$'"; undef }>,
                ...
                $s_matched,                                                      #  4
                $e_replacement,                                                  #  5
                '$INFORMIXV6ALS::re_r=eval $INFORMIXV6ALS::re_r; ' x $e_modifier,                  #  6
                $variable,                                                       #  7
                $prematch,                                                       #  8
            );
        }
...
expand wildcard arguments supplied to programs. Instead, import of Einformixv6als.pm
works well.
...
Regular expression works as character-oriented that has no /b modifier.
  ...
  s/regexp/replacement/   eval{ Einformixv6als::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
 
  $_ !~                   ($_ !~ m/ditto$Einformixv6als::matched/) ?
  s/regexp/replacement/   1 :
                          eval{ Einformixv6als::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; undef }
...
Regular expression works as byte-oriented that has /b modifier.
  ...
  s/regexp/replacement/b  eval{ Einformixv6als::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
...
Definitions in Einformixv6als.pm.
  ...
  ${Einformixv6als::eS}             qr{(?:\xFD[\xA1-\xFE][\xA1-\xFE]|[\x81-\x9F\xE0-\xFC][\x00-\xFF]|[^\x81-\x9F\xE0-\xFD\s])}
  ...
  ${Einformixv6als::not_space}      qr{(?:\xFD[\xA1-\xFE][\xA1-\xFE]|[\x81-\x9F\xE0-\xFC][\x00-\xFF]|[^\x81-\x9F\xE0-\xFD\s\x0B])}
...
=item * Returns Next Character
  ...
  If you import getc "use INFORMIXV6ALS qw(getc);", getc of your script will be rewritten in
  INFORMIXV6ALS::getc. INFORMIXV6ALS::getc is not compatible with getc of JPerl.
...
=item * Substr by INFORMIX V6 ALS Character
  ...
  If Perl version 5.14 or later, you can use the INFORMIXV6ALS::substr() function as an
...
=item * Filename Globbing
  ...
  This operator is implemented via the Einformixv6als::glob() function. See Einformixv6als::glob of
  Einformixv6als.pm for details.
